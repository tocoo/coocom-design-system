...
WCAG 2.2 AA・タップ領域 **48px** (§4.2)・代替テキスト・色だけで伝えないを下限とする。例外は本書と [labels-tags.rental-car.md](labels-tags.rental-car.md) に明示したものに限る。
...
- [決定] 指で操作する要素 (ボタン・チップ・プルダウン・オーバーレイ内の選択肢行・タブ等) の**最小の高さを 48px** (`{size.tapTarget}` = `{spacing.12}`) とする。44px を用いない
- [事実] 本節は 2026-09-15 に**従前の 44px を 48px へ改めた** (Task 009-72・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §38)。従前は §2.3 品質下限・[components.md](components.md) 共通事項ほかで 44px を下限としつつ、Button の SP 全幅 CTA のみ 48px とするなど 2 つの値が混在していた。国内宿泊 (travel) が 48px へ統一したことに揃え、2 サービスで指で押しやすさの基準を同一にする
- [事実] `{size.tapTarget}` は既存 primitive `spacing.12` (48px) を参照する用途トークンであり、**新しい数値は追加していない**。Component は primitive を直接参照しないため、用途トークンとして semantic に置く
- [事実] 未確定事項の一覧の「最小タップ領域 (44px) の spacing トークン」は本節の 48px 確定により解消する。**44px 段 (`spacing` step 11) は追加しない**
...
- [決定] 品質の下限 (適用外でも守る): コントラスト AA (支給画像内の文字を除く)・タップ領域 `48px`・代替テキスト・色だけで伝えない
...
| Modal の form の a11y・実装 API 名 | ❓ 未定義 (§7.1)。a11y は DS 層で決定しない。最小タップ領域は Task 009-72 で **48px** (`size.tapTarget` = `{spacing.12}`) に確定した (§4.2)。**44px 段 (`spacing` step 11) は追加しない** (従前どおり)。**`sheet` の最大高 (`90vh`) と `popover` の幅 (`240〜320px`) は ✅ 確定した** (2026-09-07・Web部責任者判断・判断 F-6・正本 = §7.1) | 実装実査 / 実装側タスク |
