...
- **Button の disabled は Task 009-79 で確定した** ([components.md](components.md) §1) — 非活性の CTA は**既定の意匠のまま要素全体の不透明度を `disabled.opacity` (0.5) にする**。面色・文字色を非活性専用の値へ差し替えない
- `🚧` **残る未確定**: どの variant のボタンを非活性で出すか・文言・高さ・形
- 不透明度を掛ける対象は**非活性の CTA そのものに限る**。写真・価格・カードの面へは掛けない (写真を opacity で落とす旧実装は採らない)
- 押せない理由 (満車・受付終了) は色・状態だけで伝えず、**文言で示す** ([design.md](design.md) §2.3 の品質下限)
...
- `🚧` **残る未定義**: C カテゴリ・特集ラベルのアイコンのサイズ (C1)・E2 満車・受付終了の**ボタンの variant・文言・高さ・形** (Button の disabled 自体は Task 009-79 で確定)・F 予約条件「得」の文字色とリンクの見分け
...
| 2026-09-19 | Task 009-79: **E2 満車・受付終了の前提であった Button の `disabled` が確定したことの反映** ([Issue #202](https://github.com/tocoo/coocom-design-system/issues/202)・記録 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §45)。①**E2 節**の「DS に Button の disabled 定義がないため、そちらが先に必要」を削除し、**非活性の CTA は既定の意匠のまま要素全体の不透明度を `disabled.opacity` (0.5) にする**旨 ([components.md](components.md) §1) へ改めた。**面色・文字色を非活性専用の値へ差し替えない**。②不透明度を掛ける対象は**非活性の CTA そのものに限り**、写真・価格・カードの面へは掛けない旨を明記した (写真を opacity で落とす旧実装を採らない従前の [決定] は維持)。③押せない理由は色・状態だけで伝えず**文言で示す**旨を追記した。④**残る未確定**を「E2 の意匠値」から「**どの variant のボタンを非活性で出すか・文言・高さ・形**」へ改めた (§4 の残る未定義の行を含む)。**不変**: A〜D・F 各節の全記述、E1 残室僅少の色・置き場所・実測値、ラベル・タグの器 `label.*`、token の値・参照先・`$status` (本 Task の追加 1 件を除く)、travel / inbound の成果物。**行っていないもの**: E2 の variant・文言・高さ・形の決定、ラベルの新設、実装ファイルの変更。影響度 = **高** (判定者 = Web部責任者・判定日 2026-09-19・本件について明示取得。必要レビュー主体 = Web部責任者およびチーフデザイナー)。改訂着手の設計承認取得済み (§9・§20・§45)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
