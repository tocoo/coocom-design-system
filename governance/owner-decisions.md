...
| 判断主体 | Web部責任者 |
| 判断の種別 | **現在判断** (2026-08-04 時点) |
| 取得した判断 ⓐ 会員限定ラベルの区別 | 会員限定 (有料会員限定) ラベルをカテゴリラベルと**区別する**方向を確定する。区別の手段 = 面色を用途で使い分ける — **基本 = neutral 面 / 特集紐づき = accent 面 / 例外 = inverse 面**。ただし**既存トークンの範囲で成立する組み合わせに限定**し、新しい色値・新しい面色用途を先に既成事実化しない (面色・文字色の成立条件は design.md §2.1 の検証表が正であり、面色規則の適合は Task 009-44 (#129) と重なる)。写真の上に置く場合の扱いは Task 009-46 (#131) の scrim (E-1) と関連する。用途トークン・区別規則の実定義は改訂着手の設計承認 ([review-approval-rules.md](review-approval-rules.md) §9・§20) を経た別 Task とする |
| 取得した判断 ⓑ 会員種別語の表記管轄 | 会員種別語 (無料 / 有料会員) の表記は DS の token 管轄ではなく**コンテンツ表記規則の管轄**である。その管理正本の所在は依頼 E-4 / [../services/travel/design-system/README.md](../services/travel/design-system/README.md) §16 の Open Issue に属する (本記録着手時点の現状 = 独立文書 `brand-content.md` を新設する方向・本 §16。ただし Task 009-45 (#130) で design.md §8 系への集約へ撤回する方向を別途判断する)。本記録では会員種別語の具体的な表記規則を確定しない (推測で表記規則を作らない・`brand-content.md` は未作成) |
...
| 判断後も未決・未確認の事項 | 会員限定ラベルの用途トークン・区別規則の実定義 (別 Task・§9・§20 設計承認先行)／各面色の具体トークンと文字色の成立条件 (design.md §2.1・Task 009-44)／写真上への配置 (Task 009-46 scrim)／会員種別語の管理正本の所在の確定 (§16 / Task 009-45) と具体表記。いずれも本記録では決定・補完しない |
