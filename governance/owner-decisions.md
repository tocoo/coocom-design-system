...
| 項目 | 内容 |
| --- | --- |
| 影響度 | **高** (判定者 = Web部責任者・判定日 2026-09-12・本件について明示取得) |
| 必要レビュー主体 | [review-approval-rules.md](review-approval-rules.md) §10 の影響度・高の既定 = **Web部責任者およびチーフデザイナー**。いずれか一方のレビューのみで内容レビュー完了・反映確定として扱わない (同 §10・§11) |
| 判定の位置づけ | 本判定は本件に限る都度判断であり、§8 の編集的訂正 carve-out (意味を変えない字句の修正) の適用でも、一般的な高／低の内容基準の確定でもない。primitive の追加・削除は伴わないが、**承認済みの [決定] の改訂**を伴うため carve-out の対象外である (同系の先行判断 = §31 も高) |
...
## 37. Travel の未入力状態の文字色の統一 — Task 009-68 の「プルダウンの未選択値に限る」例外の是正 — 改訂着手の設計承認・影響度・現在判断の記録

- 種別: 国内宿泊 (travel) の未入力状態の案内文字の色について、Task 009-68 (§34・2026-09-15) が定めた「**プルダウンの未選択値に限る**例外」を誤りと判断し、**UI の種別で限定せず `color.text.muted` に統一する**件について、Web部責任者が示した改訂着手の設計承認・影響度判定・現在判断の記録 ([Issue #186](https://github.com/tocoo/coocom-design-system/issues/186))。契機 = 国内レンタカーへ travel の定義体系を反映する検討の中で、レンタカーが 2026-09-07 に既に「UI の種別で限定しない」判断 (§29-3 判断 C-5) を採っており、travel 側の規則が同一の役割に異なる色を割り当てていることが判明したこと。上記 §1〜§36 とは独立した記録であり、混同しない。§5 設計承認ログ・§6 適用開始記録・§7〜§36 の各記録は変更しない。
- **本記録は §22 (2026-08-04) の記録内容を遡って変更するものではない。** §22 は当時の現在判断として保持する。本記録が是正するのは、その方向を規則化した §34 K-5 / `design.md` §2.3 の**適用範囲**である。
- **適用範囲は国内宿泊 (travel) に限る。** 本記録によって travel と rental-car の規則は同型になるが、**3 独立 DS の原則 (P1/ADR-0022) は維持する** — 値は各 DS のファイルに独立して持ち、travel が rental-car のファイルを参照・共有することはない。inbound へは及ばない。
- 規約: 恒久 Decision ID・ADR・新しい正式 Status・Phase・Gate は採番・作成・新設しない。GitHub の approval・merge を判断・設計承認と同一視しない。承認済みの [review-approval-rules.md](review-approval-rules.md) 本体は改定しない。
...
| 項目 | 内容 |
| --- | --- |
| 承認対象 | ①[../services/travel/design-system/design.md](../services/travel/design-system/design.md) §2.3 の改題・全面改訂、§2 用途表・§2.1 検証表・§2.1 の [決定] の更新、未確定事項の一覧の更新、②[../services/travel/design-system/components.md](../services/travel/design-system/components.md) の共通事項・`Input`・`Select` の整合、③[../services/travel/design-system/semantic.travel.json](../services/travel/design-system/semantic.travel.json) の `color.text.muted` / `color.text.placeholder` の `$description` / `$note` の更新 (**値・参照先・`$status` は不変**) |
| 承認種別 | **改訂着手承認** ([review-approval-rules.md](review-approval-rules.md) §9・§20) |
| 承認日 | 2026-09-15 |
| 承認主体 | Web部責任者 |
| 根拠 | 未入力状態という同一の役割に対し、UI の種別で異なる色を割り当てる根拠が正本に無い。レンタカーは同じ論点について既に限定しない判断を採っており、規則が割れていた |
| 適用範囲 | travel に限定 |
...
| 影響度 | **高** (判定者 = Web部責任者・判定日 2026-09-15・本件について明示取得) |
| 必要レビュー主体 | [review-approval-rules.md](review-approval-rules.md) §10 の影響度・高の既定 = **Web部責任者およびチーフデザイナー**。いずれか一方のレビューのみで内容レビュー完了・反映確定として扱わない (同 §10・§11) |
| 判定の位置づけ | 本判定は本件に限る都度判断であり、§8 の編集的訂正 carve-out の適用ではない。**承認済みの [決定] の改訂** (§2.3 の適用範囲・§2.1 の面上文字色の [決定]) を伴うため carve-out の対象外である |

### 37-3. 取得した現在判断 (2026-09-15)

| # | 論点 | 取得した判断 |
| --- | --- | --- |
| N-1 | 未入力状態の文字色を UI の種別で分けるか | **分けない。** `input` / `textarea` の `::placeholder`、Select の未選択値、その他の入力フィールドの未入力表示のすべてに同じ色を用いる |
| N-2 | 統一先の色 | **`color.text.muted` (`#9E9E9E`・白背景 2.68:1)。** **AA 未達であることを明示する**。適合宣言は行わない |
| N-3 | N-2 の理由 | **未入力と入力済みの判別のしやすさを優先する。** `color.text.mutedStrong` (`#616161`・6.19:1) に統一すると入力済みテキスト (`color.text.body` `#424242`) との濃度差が小さくなり、**未入力か入力済みかが読み取りにくくなる不利益の方が大きい**と判断した |
| N-4 | `color.text.placeholder` の扱い | **本記録では決定しない。** 本トークンはプレースホルダとしての用途を失うが、実装は 10 箇所以上で参照しており、その多くは選択不可の表現である。選択不可の用途トークンとして位置づけ直すか削除するかは未判定とし、`design.md` の未確定事項の一覧で追跡する。**値・参照先・`$status` は不変** |
| N-5 | 本件の影響度 (§8) | **高** (§37-2) |

### 37-4. 追加・変更するトークン

- **追加・削除はいずれも行わない。** `color.text.muted` / `color.text.placeholder` の `$description` / `$note` を更新するのみで、**`$value`・参照先・`$status` は不変**である。
- **新しい色値 (primitive) は追加していない。**
- **`$meta.version` は primitive / semantic とも据置き。**

### 37-5. 本記録が決定しないこと

- **`color.text.placeholder` を選択不可の用途トークンとして位置づけ直すか、削除するか** (N-4)。`components.md` `DateRangeCalendar` の「選択不可の文字色トークンが無い」(§36-5) と同一の論点であり、**両者は一体で判定する**。
- **実装 Repository (`tocoo/tocoo_travel`) 側の是正作業** — `input` / `textarea` の `::placeholder` を `color.text.placeholder` から `color.text.muted` へ変更する作業。範囲・順序・期限は実装 Repository 側の課題である。
- **rental-car / inbound の成果物の変更。** rental-car は既に本記録と同型の規則を持つ (§29-3 判断 C-5) ため変更は生じない。
- **適用規格・達成レベルの正式確定・適合判定・適合宣言。**
- **恒久 Decision ID・ADR・正式 Status・Phase・Gate の採番・作成・新設。**


## 38. 国内レンタカーへの横断の規則と Select の反映 — 改訂着手の設計承認・影響度・現在判断の記録

- 種別: 国内レンタカー (rental-car) へ、国内宿泊 (travel) で確定した横断の規則 (中央寄せの器の左右余白・最小タップ領域) と Select の角丸を反映する件について、Web部責任者が示した改訂着手の設計承認・影響度判定・現在判断の記録 ([Issue #188](https://github.com/tocoo/coocom-design-system/issues/188))。契機 = Web部責任者の指示「国内レンタカーについても同様に基本的にはクローン・コピーで反映させたい。レンタカー独自の UI については都度個別に定義する方針とする」(2026-09-15)。上記 §1〜§37 とは独立した記録であり、混同しない。§5 設計承認ログ・§6 適用開始記録・§7〜§37 の各記録は変更しない。
- **枠組みは §29 と同じ。** 採用するのは travel の**定義体系**であり、**値は rental-car のファイルに独立して持つ。travel のファイルを参照・共有しない** (3 独立 DS の原則 = P1/ADR-0022 は維持)。
- **適用範囲は国内レンタカー (rental-car) に限る。** travel・inbound の成果物は変更しない。
- 規約: 恒久 Decision ID・ADR・新しい正式 Status・Phase・Gate は採番・作成・新設しない。GitHub の approval・merge を判断・設計承認と同一視しない。承認済みの [review-approval-rules.md](review-approval-rules.md) 本体は改定しない。
...
| 項目 | 内容 |
| --- | --- |
| 承認対象 | ①[../services/rental-car/design-system/design.md](../services/rental-car/design-system/design.md) の §4.1 / §4.2 の新設、§5 の角丸用途トークンの 5 系統化、§2.3 品質下限・§8.3 の 44px の更新、未確定事項の一覧の更新、②[../services/rental-car/design-system/components.md](../services/rental-car/design-system/components.md) の 44px の更新と `Select` の意匠の改訂、③[../services/rental-car/design-system/semantic.rental-car.json](../services/rental-car/design-system/semantic.rental-car.json) への `radius.select` / `size.tapTarget` / `select.paddingInline` / `select.chevronSize` の追加 |
| 承認種別 | **改訂着手承認** ([review-approval-rules.md](review-approval-rules.md) §9・§20) |
| 承認日 | 2026-09-15 |
| 承認主体 | Web部責任者 |
| 根拠 | travel で確定した横断の規則が rental-car に無く、最小タップ領域は 2 サービスで値が食い違うだけでなく rental-car の DS 内でも 44px と 48px が混在していた |
| 適用範囲 | rental-car に限定 |
...
| 影響度 | **高** (判定者 = Web部責任者・判定日 2026-09-15・本件について明示取得) |
| 必要レビュー主体 | [review-approval-rules.md](review-approval-rules.md) §10 の影響度・高の既定 = **Web部責任者およびチーフデザイナー**。いずれか一方のレビューのみで内容レビュー完了・反映確定として扱わない (同 §10・§11) |
| 判定の位置づけ | 本判定は本件に限る都度判断であり、§8 の編集的訂正 carve-out の適用ではない。**承認済みの [決定] の改訂** (§2.3 品質下限の 44px・§5 の判断 F-5 の適用範囲) を伴うため carve-out の対象外である |

### 38-3. 取得した現在判断 (2026-09-15)

| # | 論点 | 取得した判断 |
| --- | --- | --- |
| P-1 | レンタカーへの反映の方針 | **基本的にクローン・コピーで反映する。** travel の定義体系を採用し、値は rental-car のファイルに独立して持つ (§29 と同じ枠組み)。**レンタカー独自の UI は都度個別に定義する** |
| P-2 | 最小タップ領域 | **48px に統一する** (`size.tapTarget` = `{spacing.12}`)。従前の 44px を改める。**44px 段 (`spacing` step 11) は追加しない** |
| P-3 | Select の角丸 | **`radius.select` (8px) とする。** `radius.action` (pill) が定める「入力要素」に対する **Select 限定の例外**であり、`Input` / `SearchForm` のフィールドへ拡大しない。**判断 F-5 (`radius.input` の廃止・入力要素の pill 統一) は維持する** |
| P-4 | 未入力状態の文字色 | **変更しない。** rental-car は 2026-09-07 (§29-3 判断 C-5) に既に「UI の種別で限定せず `color.text.muted`」を採っており、travel が 2026-09-15 (§37) に同型へ改めたため、両 DS の規則は既に一致している |
| P-5 | 進め方 | **3 本に分ける。** 1 本目 = 横断の規則と Select (本 Task)、2 本目 = 選ぶための部品、3 本目 = 一覧まわりの部品 |
| P-6 | 本件の影響度 (§8) | **高** (§38-2) |
...
| `select.chevronSize` | **追加** | `{iconSize.sm}` | bound |
| `radius.action` / `card` / `badge` / `overlay` | **不変** | — | — |
| `color.text.muted` / `label.*` | **不変** | — | — |

- **`radius.input` は Task 009-63 で削除済みであり、本 Task で復活させていない。**
- **幅ごとに値を持つ semantic トークンは新設していない** (左右余白の 5 段は `design.md` §4.1 の表で既存 `spacing` 段への割当として示す)。
- **`$meta.version` は primitive / semantic とも据置き。**

### 38-5. 本記録が決定しないこと

- **レンタカー独自の UI の仕様** (P-1)。実装 Repository (`tocoo/tocoo_rental_car`) には DS トークンを参照する scss / css が `epic-ds` ブランチを含め **1 件も存在しない** (`8e5d928a0`・2026-09-15・本 Repository で実測) ため、実装から独自 UI を洗い出すことはできない。画面要件・実物に基づき都度個別に定義する。
- **後続 2 本で移植する部品の具体的な値** (選ぶための部品・一覧まわりの部品)。
- **実装 Repository 側の適用作業。** 範囲・順序・期限は実装 Repository 側の課題である。
...
## 40. 国内レンタカーへの「一覧まわりの部品」の反映 — 改訂着手の設計承認・影響度・現在判断の記録

- 種別: 国内レンタカー (rental-car) へ「一覧まわりの部品」5 件 (OverlayTrigger・DateRangeCalendar・RangeSlider・Skeleton・StickyBar) を定義する件について、Web部責任者が示した改訂着手の設計承認・影響度判定・現在判断の記録 ([Issue #192](https://github.com/tocoo/coocom-design-system/issues/192))。**§38 P-5 が定めた 3 本のうちの 3 本目 (最終)** にあたる。上記 §1〜§39 とは独立した記録であり、混同しない。§5 設計承認ログ・§6 適用開始記録・§7〜§39 の各記録は変更しない。
- **適用範囲は国内レンタカー (rental-car) に限る。** travel・inbound の成果物は変更しない。値は rental-car のファイルに独立して持つ (3 独立 DS の原則 = P1/ADR-0022 は維持)。
- 規約: 恒久 Decision ID・ADR・新しい正式 Status・Phase・Gate は採番・作成・新設しない。GitHub の approval・merge を判断・設計承認と同一視しない。承認済みの [review-approval-rules.md](review-approval-rules.md) 本体は改定しない。
...
| 項目 | 内容 |
| --- | --- |
| 承認対象 | ①[../services/rental-car/design-system/components.md](../services/rental-car/design-system/components.md) への 5 節の新設と既存記述 (`Filter` / `Sort / Pagination`) の整合、②[../services/rental-car/design-system/semantic.rental-car.json](../services/rental-car/design-system/semantic.rental-car.json) への器トークン群 `trigger.*` / `calendar.*` / `rangeSlider.*` / `skeleton.*` / `stickyBar.*` の追加 |
| 承認種別 | **改訂着手承認** ([review-approval-rules.md](review-approval-rules.md) §9・§20) |
| 承認日 | 2026-09-15 |
| 承認主体 | Web部責任者 |
| 根拠 | 5 件はいずれも画面に存在するが、既存記述は用途を述べるにとどまり寸法・状態・禁止事項が定まっていない |
| 適用範囲 | rental-car に限定 |
...
| 影響度 | **高** (判定者 = Web部責任者・判定日 2026-09-15・本件について明示取得) |
| 必要レビュー主体 | [review-approval-rules.md](review-approval-rules.md) §10 の影響度・高の既定 = **Web部責任者およびチーフデザイナー**。いずれか一方のレビューのみで内容レビュー完了・反映確定として扱わない (同 §10・§11) |
| 判定の位置づけ | 本判定は本件に限る都度判断であり、§8 の編集的訂正 carve-out の適用ではない。Component を 5 件新設するため carve-out の対象外である |

### 40-3. 取得した現在判断 (2026-09-15)

| # | 論点 | 取得した判断 |
| --- | --- | --- |
| R-1 | 一覧まわりの部品 5 件を定義するか | **定義する。** 値の根拠は国内宿泊 (travel) を正とし、値は rental-car のファイルに独立して持つ |
| R-2 | travel の `AvailabilityCalendar` を移植するか | **移植しない。** 日付ごとに空室と料金を一覧するカレンダーは宿泊固有であり、レンタカーに該当する画面が無い (本 Repository で `components.md` を確認) |
| R-3 | `DateRangeCalendar` の範囲 | **日付の範囲のみを定める。** レンタカーは出発・返却に**時刻**を伴うが、`SearchForm` の未確定事項「日時の入力方式 (native か Modal の form か)」は本 Task で決定しない |
| R-4 | 本件の影響度 (§8) | **高** (§40-2) |

### 40-4. 追加・変更するトークン

**semantic (rental-car のみ)** — いずれも既存 primitive または既存 semantic (`radius.action`) への参照であり、**新しい数値・新しい色値・新しい primitive は追加していない**。

| 群 | 件数 | 主な参照先 |
| --- | --- | --- |
| `trigger` (起点ボタンの器) | 7 | `{radius.action}` / `{spacing.2}` / `{spacing.4}` / `{spacing.5}` / `{border.width.thin}` / `{typography.size.sm}` |
| `calendar` (日付カレンダーの器) | 8 | `{spacing.4}` / `{spacing.12}` / `{spacing.6}` / `{spacing.8}` / `{typography.size.xs}` / `{typography.size.sm}` / `{radius.lg}` / `{border.width.thin}` |
| `rangeSlider` (レンジスライダーの器) | 5 | `{spacing.1}` / `{radius.full}` / `{spacing.4}` / `{border.width.thick}` / `{spacing.6}` |
| `skeleton` (骨組みの器) | 7 | `{radius.sm}` / `{spacing.4}` / `{spacing.3}` / `{spacing.6}` / `{spacing.8}` / `{spacing.10}` |
| `stickyBar` (追従帯の器) | 5 | `{spacing.1}` / `{spacing.4}` / `{spacing.3}` / `{typography.size.sm}` / `{typography.size.xs}` |
...
- **`DateRangeCalendar` と時刻の入力方式の接続** (R-3)。`SearchForm` の未確定事項として残す。
- **選択不可 (過去日) の文字色トークン。** 実装・デザインとも不透明度を掛けた暫定表現であり、実効コントラストを本 Repository で検証していない。**AA 適合を主張しない**。未確定事項として起票するにとどめ、トークンの新設は行わない。
- **影 (`shadow.*`) と遷移 (`motion.*`)・`radius.card` の実値。** placeholder のまま維持する。
- **実装 Repository 側の適用作業。** 範囲・順序・期限は実装 Repository 側の課題である。
- **travel・inbound の成果物の変更。**
- **適用規格・達成レベルの正式確定・適合判定・適合宣言。**
- **恒久 Decision ID・ADR・正式 Status・Phase・Gate の採番・作成・新設。**
...
## 41. 最小タップ領域を押した結果と環境で分ける — 改訂着手の設計承認・影響度・現在判断の記録

- 種別: 国内宿泊 (travel) と国内レンタカー (rental-car) の最小タップ領域について、単一値 48px (`{size.tapTarget}`) を**押した結果による 5 区分 × `breakpoint.lg` (1024px) を境とする 2 環境**へ改める件の、Web部責任者が示した改訂着手の設計承認・影響度判定・現在判断の記録 ([Issue #194](https://github.com/tocoo/coocom-design-system/issues/194))。上記 §1〜§40 とは独立した記録であり、混同しない。§5 設計承認ログ・§6 適用開始記録・§7〜§40 の各記録は変更しない。
- **適用範囲は travel と rental-car の 2 サービス。** inbound は `size.tapTarget` を持たないため対象外 (本 Repository で実測)。値は各 DS のファイルに独立して持つ (3 独立 DS の原則 = P1/ADR-0022 は維持)。
- 規約: 恒久 Decision ID・ADR・新しい正式 Status・Phase・Gate は採番・作成・新設しない。GitHub の approval・merge を判断・設計承認と同一視しない。承認済みの [review-approval-rules.md](review-approval-rules.md) 本体は改定しない。
...
| 項目 | 内容 |
| --- | --- |
| 承認対象 | ①[../services/travel/design-system/design.md](../services/travel/design-system/design.md) §4.2 と [../services/rental-car/design-system/design.md](../services/rental-car/design-system/design.md) §4.2 の書き換え、②両 `semantic.*.json` の `size.tapTarget` (単一・bound) の廃止と 5 件への分割、③両 [components.md](../services/travel/design-system/components.md) の参照の割当て直しと共通事項への区分表の追加 |
| 承認種別 | **改訂着手承認** ([review-approval-rules.md](review-approval-rules.md) §9・§20) |
| 承認日 | 2026-09-15 |
| 承認主体 | Web部責任者 |
| 根拠 | 従前の単一値は、押した結果 (確定・遷移するのか、もう一度押せば戻るのか)・押す実体 (面を持つ単独要素か、行全体か、密集した格子のセルか)・環境 (指かマウスか) を区別せず全操作要素へ掛かっており、テキスト入力欄 (`Input`) まで同じ値で縛っていた |
...
| 影響度 | **高** (判定者 = Web部責任者・判定日 2026-09-15・本件について明示取得) |
| 必要レビュー主体 | [review-approval-rules.md](review-approval-rules.md) §10 の影響度・高の既定 = **Web部責任者およびチーフデザイナー**。いずれか一方のレビューのみで内容レビュー完了・反映確定として扱わない (同 §10・§11) |
| 判定の位置づけ | 本判定は本件に限る都度判断であり、§8 の編集的訂正 carve-out の適用ではない。承認済み成果物 (`design.md`) の規則本体を書き換え、`$status: bound` のトークンを廃止するため carve-out の対象外である |

### 41-3. 取得した現在判断 (2026-09-15)

| # | 論点 | 取得した判断 |
| --- | --- | --- |
| S-1 | 最小タップ領域を単一値で定めるか | **単一値では定めない。** 押す実体と押した結果による 5 区分に分け、区分ごとに値を持つ |
| S-2 | 区分を何で分けるか | **「押す実体 (面を持つ単独要素 / 行全体 / 密集した格子のセル / 連続値の操作子)」×「押し間違えたときに取り消せるか (その場で押し直せる / 確定・遷移・再取得が走る)」に加え、環境 (モバイルかデスクトップか) を第 3 の軸とする** |
| S-3 | 区分と値 | **A 確定・遷移 48 / 40px・B 開くだけ 48 / 32px・C 戻せる切替 40 / 32px・D 格子のセル 40 / 32px・E 連続値の操作子 24 / 24px** (左 = 1024px 未満・右 = 1024px 以上)。実寸はいずれも既存 `spacing` の段であり新しい数値は追加しない |
| S-4 | 区分を Component ごとに固定するか | **固定しない。押した結果で決める。** 同じラジオでも、オーバーレイの中で選んだ瞬間に確定して閉じる `OptionRow` は A、フォームの中で押しても確定しない `StorePicker` は C とする |
| S-5 | `Input` (テキスト入力欄) の扱い | **本規則の対象外とする。** 押しても文字カーソルが入るだけで押し間違いによる損失が無く、高さは文字サイズと上下余白で決まる。高さ 48px はフォーム要素の見た目を揃えるための値として維持する |
| S-6 | 環境を分ける境界 | **`breakpoint.lg` (1024px) の 1 つのみ。** `pointer: coarse` 等のポインタ種別による分岐は採らない (幅で切っている `design.md` §3.1 の見出しの段下げ・§7.1 の Modal の表示形態の切替と境界が食い違い、実機でどちらが効いているかを切り分けられなくなるため) |
| S-7 | 1024px 以上の値をトークン化するか | **トークン化せず `design.md` §4.2 の表で示す。** 幅ごとの値をトークンに持たせない扱いは §3.1 の見出しスケール・§4.1 の中央寄せの器の左右余白と同一 |
| S-8 | 本件の影響度 (§8) | **高** (§41-2) |

### 41-4. 追加・変更するトークン

**semantic (travel・rental-car の両方)** — **廃止 1 件・追加 5 件**。いずれも既存 primitive `spacing` の段への参照であり、**新しい数値・新しい primitive は追加していない**。**44px 段 (`spacing` step 11) は追加していない**。

| トークン | 値 (1024px 未満) | 参照先 | 1024px 以上 (表で示す) |
| --- | --- | --- | --- |
| `size.tapTarget` | — | — | **廃止** (単一値 48px・`$status: bound` だった) |
| `size.tapTarget.commit` | 48px | `{spacing.12}` | 40px |
| `size.tapTarget.open` | 48px | `{spacing.12}` | 32px |
| `size.tapTarget.toggle` | 40px | `{spacing.10}` | 32px |
| `size.tapTarget.cell` | 40px | `{spacing.10}` | 32px |
| `size.tapTarget.thumb` | 24px | `{spacing.6}` | 24px |

- **`$meta.version` は primitive / semantic とも据置き。**
- 器トークン群 (`trigger` / `calendar` / `row` / `chip` / `tab` / `select` / `rangeSlider`) の `$description` は参照先の区分を示すよう更新したが、**値・参照先・`$status` は不変**。

### 41-5. 本記録が決定しないこと

- **`pointer: coarse` 等のポインタ種別による分岐の将来の採否** (S-6 は現時点で採らないことのみを定める)。
- **`spacing` への 44px 段の追加** (従前どおり追加しない = §18)。
- **`Input` の高さ 48px の妥当性そのもの。** 本記録はタップ領域の規則の対象外とすることのみを定める。
- **実装 Repository 側の適用作業。** 範囲・順序・期限は実装 Repository 側の課題である。
- **inbound の成果物の変更。**
- **適用規格・達成レベルの正式確定・適合判定・適合宣言。**
- **恒久 Decision ID・ADR・正式 Status・Phase・Gate の採番・作成・新設。**
...
## 44. コントラストの品質下限を横断規約へ移し、テキストの役割による段階制へ改める — 改訂着手の設計承認・影響度・現在判断の記録

- 種別: コントラストの品質下限について、**下限の決め方を 3 サービス横断の運用規約 (`contrast-rules.md`) へ移し、テキストの役割による段階制へ改める**件の、Web部責任者が示した改訂着手の設計承認・影響度判定・現在判断の記録 ([Issue #200](https://github.com/tocoo/coocom-design-system/issues/200))。上記 §1〜§43 とは独立した記録であり、混同しない。§5 設計承認ログ・§6 適用開始記録・§7〜§43 の各記録は変更しない。
- **適用範囲は travel・rental-car・inbound の 3 サービスおよび Governance。**
- 本節は **§23 (デザインシステム適用範囲の例外を Governance 横断ルールとして新設することの受理と分類) が別 Task へ送った「Governance 原則正本の実本文の作成」を、本件について実施するもの**である。§23 の対象 (広告・非 UI クリエイティブ) とは別の論点であり、§23 の記録内容は変更しない。
- 規約: 恒久 Decision ID・ADR・新しい正式 Status・Phase・Gate は採番・作成・新設しない。GitHub の approval・merge を判断・設計承認と同一視しない。承認済みの [review-approval-rules.md](review-approval-rules.md) 本体は改定しない。
...
| 項目 | 内容 |
| --- | --- |
| 承認対象 | ①`governance/contrast-rules.md` の新設 (Governance 原則正本領域への追加)、②3 サービス `design.md` の品質下限の記述を同規約への参照へ改めること、③rental-car の accent 関連記述の矛盾の是正 |
| 承認種別 | **改訂着手承認** ([review-approval-rules.md](review-approval-rules.md) §9・§20) |
| 承認日 | 2026-09-19 |
| 承認主体 | Web部責任者 |
| 根拠 | 品質下限が「WCAG 2.2 AA を最低ラインとし違反値を入れない」という**二値**で書かれている一方、DS は AA 未達の組み合わせを 4 件運用しており、規定と運用の実態が一致していない。例外が**用途名の列挙**で書かれているため、列挙に無い表示要素が現れるたびに Owner 判断を要する状態が続いていた |
| 契機 | 2026-09-19 の Slack 指摘 — 国内レンタカー検索結果の「お得額」(他社比較の値引額・「他社より N 円お得」) の文字色が `color.accent.campaign` `#E4572E` (白面上 **3.68:1**・AA 未達) であることの可否を spec-kit が問い、Web部責任者が「見送りでよい (現状のまま)」と回答した (2026-09-19 11:27)。**その後、同種の指摘が都度発生する状態そのものが問題であるとして、色を個別に指定する改定ではなくコントラスト規定を横断で改める方向が示された (同日)** |
| 適用範囲 | travel・rental-car・inbound の 3 サービスおよび Governance |

### 44-2. 影響度 (§8)

| 項目 | 内容 |
| --- | --- |
| 影響度 | **高** (判定者 = Web部責任者・判定日 2026-09-19・本件について明示取得) |
| 必要レビュー主体 | [review-approval-rules.md](review-approval-rules.md) §10 の影響度・高の既定 = **Web部責任者およびチーフデザイナー**。いずれか一方のレビューのみで内容レビュー完了・反映確定として扱わない (同 §10・§11) |
| 判定理由 | Governance 横断の原則正本の**初の新設** ([README.md](README.md) が「本 bootstrap では新規作成しない」とする principles 正本領域への初の追加) であり、かつ 3 サービスの `design.md` の品質下限を実改訂するため。高／低の一般的な明文判定基準は未整備であり推測・補完しない (同 §8・§21)。同 §8 の編集的訂正 carve-out は規範方向の追加・文の追加を含むため本件に適用しない |

### 44-3. 取得した現在判断 (2026-09-19)

| 記号 | 判断 | 内容 |
| --- | --- | --- |
| H-1 | **色を個別に指定する改定は採らない** | 「お得額」に用途トークンや専用の例外条項を与える案は採らない。**コントラストの規定そのものを横断レイヤーで改め、各 DS の内部で適切に取り回せる状態にする** |
| H-2 | **下限はテキストの役割で決まる** | 役割 1 必須情報 = 4.5:1 (取得時点の表現は「例外なし」。**H-7 で個別例外条項との関係を明確化した**)／役割 2 大きなテキスト相当 = 3:1／役割 3 冗長表現 = 3:1 (AA 未達を明示)／役割 4 非テキスト UI 要素 = 3:1／役割 5 装飾・非情報 = 下限なし |
| H-3 | **役割 3 は一般条件で判定する** | 冗長性 (同じ情報が同一画面の他の要素からも読み取れる)・色に依存しない (文言併記)・3:1 は満たす・AA 未達を明示する の 4 条件をすべて満たすこと。**用途名を列挙しない。判定に Owner の都度判断を要しない** |
| H-4 | **3:1 未満は役割に収まらない** | 3:1 を下回る組み合わせは個別の例外条項として当該 DS に明記し、その新設は Owner の明示的判断を要する |
| H-5 | **「お得額」の扱い** | 役割 3 に該当する (価格本体が同一画面に別途表示され、文言「お得」を併記し、白面上 3.68:1 で 3:1 を満たす)。**現行の `#E4572E` のまま許容し、AA 未達を明示する。文字色・文字サイズ・トークンを変更しない** |
| H-6 | **目標レベルは AA のまま維持する** | 目標とする達成レベル (WCAG 2.2 AA = 要求仕様 R9) は変更しない。ただし**役割 3 は WCAG 1.4.3 の適用除外 (装飾・非表示・画像の一部) には該当せず、AA 未達を明示したうえでの限定的な許容である** — 「AA を満たしている」と言い換えない。役割 3 を設ける理由は、AA 未達の運用が既に複数存在する実態に対し許容の範囲と条件を事前に定めて予測可能にすることにあり、用途ごとの個別判断を積み増す従前の運用より逸脱の総量を抑えられると判断した |
| H-7 | **役割 1 の 4.5:1 未達は、個別例外条項として明記したものに限り認める** | H-2 の「例外なし」と、現に維持している AA 未達の運用 (A 割引率ラベルに限る白文字 2.78:1 / 2.15:1・未入力状態の案内文字 2.68:1) が矛盾するため明確化した。**当該 DS の `design.md` に個別の例外条項として明記したものに限り役割 1 の下限未達を許容し、それ以外は認めない**。新設に Owner の明示的判断を要する点 (H-4) は変えない。**既存 2 件の適用範囲・実測値・トークン・文字色は変更しない**

### 44-4. 実測の記録 (本 Repository で実施・2026-09-19)

WCAG 2.x 標準式により算出。DS 記載の既存値と一致することを確認した。

| 文字 | 面 | 実測 | 通常テキスト 4.5:1 |
| --- | --- | ---: | --- |
| `#E4572E` (campaign accent 主色) | `#FFFFFF` | 3.68:1 | 未達 |
| `#E4572E` | `#F9F9F9` | 3.50:1 | 未達 |
| `#E4572E` | `#F5F5F5` | 3.38:1 | 未達 |
| `#E4572E` | `#212121` | 4.37:1 | 未達 |
| `#E4572E` | `#F9CDBC` (accent 淡色面) | 2.54:1 | 未達 |
| `#8A2E11` (accent 濃色段・**🚧 仮色**) | `#FFFFFF` | 8.47:1 | 達成 (仮色のため確定値ではない) |
| `#8A2E11` (**🚧 仮色**) | `#F5F5F5` | 7.77:1 | 達成 (同上) |

- `#8A2E11` は実査待ちの **🚧 仮色**であり、本記録は同値を bound へ昇格させない。実色確定後に役割 3 で運用している組み合わせを AA 適合の濃色段へ寄せるかは ❓ 未判定。

### 44-5. 本記録が決定しないこと

- **適用規格・達成レベルの正式確定、適合判定、適合宣言。** 本記録および新設する規約はこれらを行わない。
- **既存の例外条項 4 件の適用範囲・実測値・トークンの変更** (A 割引率ラベルの白文字・未入力状態の案内文字・accent を文字色として明色面に置く場合・C ラベルの枠線)。位置づけを与えるのみである。
- **未入力状態の案内文字 (`#9E9E9E`・2.68:1) の最終的な扱い。** 3:1 を下回るため役割 3 に収まらず、個別例外として維持する。3:1 を満たす色へ寄せるかは ❓ 未判定。
- **accent 濃色段 (`#8A2E11` ほか) の実色の確定**、および `🚧` 仮色の bound 昇格。
- **画像・グラデーション面上の文字のコントラスト評価方法** (scrim の要否・評価点の取り方)。❓ 未定義のまま残す。
- **ブランド色そのものの値の変更。**
- **コントラスト以外の A11y 品質下限** (タップ領域・代替テキスト・色だけで伝えない) の変更。
- **既存実装の是正。** 範囲・順序・期限は実装 Repository 側の課題である。
- **§23 の記録内容の変更**、および §23 が対象とする適用範囲の例外原則 (広告・非 UI クリエイティブ) の正本の作成。
- **恒久 Decision ID・ADR・正式 Status・Phase・Gate の採番・作成・新設。**


## 45. 押せない状態 (`disabled`) の意匠を 3 サービスで定義する — 改訂着手の設計承認・影響度・現在判断の記録

- 種別: Component の状態 `disabled` (押せない状態) の意匠値を 3 サービスの Design System に定義する件の、Web部責任者が示した改訂着手の設計承認・影響度判定・現在判断の記録 ([Issue #202](https://github.com/tocoo/coocom-design-system/issues/202))。上記 §1〜§44 とは独立した記録であり、混同しない。§5 設計承認ログ・§6 適用開始記録・§7〜§44 の各記録は変更しない。
- **適用範囲は travel・rental-car・inbound の 3 サービス。**
- 規約: 恒久 Decision ID・ADR・新しい正式 Status・Phase・Gate は採番・作成・新設しない。GitHub の approval・merge を判断・設計承認と同一視しない。承認済みの [review-approval-rules.md](review-approval-rules.md) 本体は改定しない。
...
| 項目 | 内容 |
| --- | --- |
| 承認対象 | ①3 サービスの `components.md` への `disabled` の意匠の定義、②`primitive.*.json` / `semantic.*.json` への不透明度トークンの追加、③各 `design.md`・`labels-tags.rental-car.md`・見本ページの未取得記述の是正 |
| 承認種別 | **改訂着手承認** ([review-approval-rules.md](review-approval-rules.md) §9・§20) |
| 承認日 | 2026-09-19 |
| 承認主体 | Web部責任者 |
| 根拠 | `disabled` は命名規則§2 の状態の固定リストに含まれるにもかかわらず、3 サービスとも意匠値が未定義 (`🚧`) であった。`components.md` §1 は「Component が定義するのは自身の構造・**状態**・トークン参照」と宣言しており、宣言した定義範囲の内側が空いている状態であった |
| 契機 | 2026-09-19 — Web部責任者が Button の `disabled` の定義の不在を指摘し、本 Repository の全走査によって 3 サービスとも未定義であること、および実装 (`tocoo/tocoo_travel` `origin/dev-ds`) が不透明度 `.5` / 不透明度 `.45` + 文字色 / 文字色のみの 3 通りに割れていることを確認した |
| 適用範囲 | travel・rental-car・inbound の 3 サービス |
...
| 影響度 | **高** (判定者 = Web部責任者・判定日 2026-09-19・本件について明示取得) |
| 必要レビュー主体 | [review-approval-rules.md](review-approval-rules.md) §10 の影響度・高の既定 = **Web部責任者およびチーフデザイナー**。いずれか一方のレビューのみで内容レビュー完了・反映確定として扱わない (同 §10・§11) |
| 判定理由 | 3 サービスの Component 定義と token に及ぶため。同 §8 の編集的訂正 carve-out は「Component 定義・token 値に影響する変更」を明文で除外しており、本件に適用しない。高／低の一般的な明文判定基準は未整備であり推測・補完しない (同 §8・§21) |

### 45-3. 取得した現在判断 (2026-09-19)

| 記号 | 判断 | 内容 |
| --- | --- | --- |
| I-1 | **定義はサービスごとに個別に置く** | `disabled` の横断規約 (Governance 原則正本) は新設しない。3 サービスそれぞれの `components.md` と token に定義する。**3 独立 DS の原則 (P1/ADR-0022) を維持する** |
| I-2 | **表現は要素全体の不透明度を下げる方式とする** | 押せない状態専用の面色・文字色トークンは**新設しない**。既定状態の意匠をそのまま薄くする。実装が既に採っている方式を正式採用するものである |
| I-3 | **不透明度の値は 0.5 に揃える** | 用途 (ボタン／セル等) によって段階を分けない。実装に併存する `.45` は `.5` へ揃える |
| I-4 | **押せない理由は文字で併記する** | 不透明度の低下だけで理由を伝えたものとして扱わない。色だけで伝えない (WCAG 2.2 AA / R9) の品質下限は従前どおり適用する |
| I-5 | **コントラストの扱い** | 不透明度を掛けた結果は [contrast-rules.md](contrast-rules.md) の役割 1・役割 4 のいずれの下限も下回る。WCAG 2.2 が**無効化された利用者インタフェース部品をコントラスト要件の対象外**とすることを根拠に許容し、**適合宣言は行わない** (同規約 §7)。個別例外条項 (同規約 §6) の新設には**あたらない** — 同 §6 は役割の下限を満たさない**テキストの組み合わせ**についての扱いであり、規格の対象外である無効化部品はその対象ではない |

### 45-4. 実測の記録 (本 Repository で実施・2026-09-19)

WCAG 2.x 標準式により算出。不透明度 0.5 を白地 (`#FFFFFF`) の上で合成した後の値である。

| サービス | 主ボタンの面 | 合成後の面 | 白文字とのコントラスト |
| --- | --- | --- | ---: |
| travel / rental-car | `#2C50C8` | `#96A8E4` 相当 | 2.33:1 |
| inbound | `#9E2334` | `#CE9199` 相当 | 2.58:1 |

- 参考 (採用しなかった値): 不透明度 `0.45` の場合、travel / rental-car の主ボタンは **2.16:1** となる。
- 参考 (採用しなかった方式): 面 `#F5F5F5` + 文字 `#9E9E9E` = **2.46:1**、面 `#F5F5F5` + 文字 `#616161` = **5.68:1**。

### 45-5. 本記録が決定しないこと

- **「選べない値」の表現。** カレンダーの過去日・満室 / 満車のセルのような、操作要素ではない値の選択不可表現は本記録の対象外とする。`color.text.placeholder` の位置づけ (§37 N-4) および `components.md` `DateRangeCalendar` の「選択不可の文字色トークンが無い」(§36-5) と**一体で判定する**論点として据え置く。
- **E2 満車・受付終了 / E2 販売終了の意匠。** 非活性の CTA が `disabled` の規則に従うことは確定するが、**どの variant のボタンを非活性で出すか・文言・高さ・形**は未確定のまま残す。
- **`active` / `loading` / `error` / `success` の実体** (follow-up #4 / #2)。
- **`hover` の暫定参照 (`opacity` ≈ .85) の確定。** 本記録は `disabled` の不透明度のみを定める。
- **既存実装の是正。** 範囲・順序・期限は実装 Repository 側の課題である。`.45` を `.5` へ揃える作業を含む。
- **適用規格・達成レベルの正式確定、適合判定、適合宣言。**
- **恒久 Decision ID・ADR・正式 Status・Phase・Gate の採番・作成・新設。**
