...
- 未確定事項: 実px (角丸/影/余白)・8スロット対応付け・画像欠落時の fallback ❓ (Card 実装着手時) / バッジの実px (`radius.badge` は暫定 sm) 🚧 / 割引率の算出式・端数処理・上限値は上流未決 (`design.md` §8.1) / **1 施設に複数の料金選択肢を並べる横並び構成 (食事条件別の選択肢リスト等) が 8 スロット構造 (単一の `Card.slot.price` を前提) で表現できるか** ❓ (依頼元 2026-08-03・依頼 D-8。Task 009-39 で受理。既存の実px・8スロット対応付けの実査待ちとは別論点) / 会員限定 (有料会員限定) ラベルをカテゴリラベルと区別するか 🚧 (依頼元 2026-08-04・依頼A。Task 009-43 で受理。**区別する方向** = 面色使い分け 基本 neutral 面 / 特集 accent 面 / 例外 inverse 面・既存トークンの範囲に限定・実定義は別 Task・正本 = `governance/owner-decisions.md` §19。面色・文字色の成立条件は `design.md` §2.1・Task 009-44 と重なる) / バッジ/ラベルの白文字非準拠 (割引率 `#C8912C` 面/白・小サイズ campaign accent 面/白) の扱い 🚧 (依頼元 2026-08-04・依頼B。Task 009-44 で受理。現行の面色規則 (逆色面 `{color.text.strong}` 固定・白文字を使用しない・(i)(ii) に campaign accent を面として使わない) に不適合。**非操作の点的ラベル + campaign accent/scheme 逆色面 に限り白文字を例外許容する方向** = サイズ/ウェイト不問・AA 未達明示・実規則化は別 Task・現行規則は維持・正本 = `governance/owner-decisions.md` §20) / 割引率の表記の基本形が §8.1 で `-NN%` → `NN%OFF` へ変更される方向 🚧 (依頼元 2026-08-04・Task 009-45・Web部責任者判断 2026-08-04・§8.1 本体の実改訂は別 Task・正本 = `governance/owner-decisions.md` §21。本 Card の §8.1 参照 (Do「割引率の表記は `design.md` §8.1 に従う (`-NN%`)」・共通事項の割引率表記行) は §8.1 実改訂時に `NN%OFF` へ整合予定・現時点では未改訂)
