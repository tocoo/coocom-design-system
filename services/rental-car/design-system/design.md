...
- [観察] 3サービス中で素材が最も揃うが、ResultCard の実 px・料金列の実ラベルが未取得である (ボタンの disabled は Task 009-79 で確定した)
...
| `color.scheme.main.tint` `#E8EDFB` | `color.scheme.main.base` `#2C50C8` (= `color.text.link`。主色文字) | 5.81:1 | 達成 | 達成 | 許可 (StepIndicator 完了・SecretPrice のマスク面の文字・Filter パネルの選択行 = [components.md](components.md)) |
...
| `color.surface.default` `#FFFFFF` | `color.text.mutedStrong` `#616161` | 6.19:1 | 達成 | 達成 | 許可 (判読性を要する補助情報 = §2.3) |
| `color.surface.default` `#FFFFFF` | `color.text.muted` `#9E9E9E` | 2.68:1 | 未達 | 未達 | **例外として許可** (§2.6 の未入力状態の案内文字に限る。AA 未達を明示したうえでの許容であり適合宣言ではない。判読性を要する情報・装飾以外の本文には用いない) |
| `color.surface.default` `#FFFFFF` | `color.text.link` `#2C50C8` | 6.80:1 | 達成 | 達成 | 許可 (リンクの既定 = §2.5) |
| `color.surface.default` `#FFFFFF` | `color.text.linkHover` `#2340A6` | 8.92:1 | 達成 | 達成 | 許可 (リンクの hover = §2.5) |
| `color.surface.default` `#FFFFFF` | `color.text.linkActive` `#1B3488` | 11.09:1 | 達成 | 達成 | 許可 (リンクの active = §2.5) |
...
- ウェイト: thin 100 / regular 400 / bold 700 / extraBold 800。**見出し・本文とも既定は変えておらず** (見出し 700 = `font.heading.weight`・本文のウェイトは正本上未定義)、`thin` / `extraBold` を参照する semantic は無い。行高: tight 1.3 (見出し = `font.heading.lineHeight`) / normal 1.5 / relaxed 1.8 (本文 = `font.body.lineHeight`)
- [事実] 上記 4 段は、書体 (LINE Seed JP) が Google Fonts で配信するウェイトと一致する (本 Repository で実測。`wght@100` / `@400` / `@700` / `@800` は `@font-face` が日本語 124 スライスぶん返り、`@200` / `@300` / `@500` / `@600` / `@900` は CSS ではなく `400: Font family not found` が返る)。**改訂前のスケール 400 / 500 / 700 / 900 のうち 500 と 900 には対応する字面が存在しなかった**ため、Task 009-65 で同 2 段を削除し、配信されている `thin` 100 と `extraBold` 800 を追加した (Issue #174・Owner 2026-09-11・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §31)。削除した 2 段を参照する semantic は本サービスに存在しなかった (実測)
...
- [決定] **新しいサイズ値・行間値 (primitive) は追加しない**。h1〜h6 はいずれも既存スケール (`4xl` / `3xl` / `2xl` / `xl` / `md`) への割当である。狭い幅の段も同じスケール内の既存段への割当である。**幅ごとの値を持つ semantic トークンも新設しない** — 狭い幅の段は上表で既存 primitive 段への割当として示す
- [事実] **ウェイト値については、Task 009-65 で `medium` (500) / `black` (900) を削除し `thin` (100) / `extraBold` (800) を追加した** (§3 の [事実])。これは書体が配信しない段を正本から除き、配信する段に揃えたものであり、**見出しの太さの既定 (700) は変更していない**。`thin` / `extraBold` を参照する semantic は設けておらず、本節の h1〜h6 はいずれも `font.heading.weight` (700) を用いる
...
ページの主内容を収める**中央寄せの器** (`max-width` + `margin-inline: auto` を持つ器) の左右余白 (`padding-inline`) を、幅の段ごとに定める。値は既存 `spacing` 段への割当であり、**新しい数値・新しいトークンは追加していない**。

| 幅 | 左右余白 | 参照する段 |
|---|---|---|
| 〜639px | 16px | `spacing.4` |
| 640〜767px | 24px | `spacing.6` |
| 768〜1023px | 24px | `spacing.6` |
| 1024〜1279px | 32px | `spacing.8` |
| 1280px〜 | 48px | `spacing.12` |

- [決定] 本表の 5 段を、中央寄せの器の左右余白の既定とする。ページごとに別の値を持たない
- [事実] 640 の段と 768 の段は同値 (24px) である。値が変わる境界は `640` / `1024` / `1280` の 3 つで、`768` は §4 の breakpoint として存在するが本表では段を移さない
- [決定] 幅ごとに値を持つ semantic トークンは新設せず、本表で既存 `spacing` 段への割当として示す。文書レベルの見出しのサイズ段 (§3.1) と同じ扱いとする
- [事実] 器の幅は §4 のコンテナ幅 (`975 / 1195 / 1425px`) が正であり、本節は左右余白のみを定める
- [事実] 本節は国内宿泊 (travel) `design.md` §4.1 の**定義体系を採用**したものである (Task 009-72・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §38)。**値は本 DS のファイルに独立して持ち、travel のファイルを参照・共有しない** (3 独立 DS の原則 = P1/ADR-0022)

### 4.2 最小タップ領域

- [決定] 最小の高さは**単一の値で定めない**。**押す実体と押した結果による 5 区分**に分け、区分ごとに**幅による 2 つの値**を定める。区分と値は下表が正である

| 区分 | 押す実体 | 押した結果 | `breakpoint.lg` (1024px) 未満 | 1024px 以上 |
|---|---|---|---|---|
| **A 確定・遷移** | 面を持つ単独要素 / 行全体 | 遷移・送信・再取得が走り、押し直しでは戻らない | **48px** (`{size.tapTarget.commit}` = `{spacing.12}`) | **40px** |
| **B 開くだけ** | 面を持つ単独要素 | 一覧・パネルが開く。閉じれば元に戻る | **48px** (`{size.tapTarget.open}` = `{spacing.12}`) | **32px** |
| **C 戻せる切替** | 行全体 / チップ | 条件が 1 つ増減する。もう一度押せば戻る | **40px** (`{size.tapTarget.toggle}` = `{spacing.10}`) | **32px** |
| **D 格子のセル** | 格子状に密集したセル | 隣接するセルを誤って押しやすい | **40px** (`{size.tapTarget.cell}` = `{spacing.10}`) | **32px** |
| **E 連続値の操作子** | つまみ | 値がずれる。ドラッグで直せる | **24px** (`{size.tapTarget.thumb}` = `{spacing.6}`) | **24px** |

- [決定] **区分は Component の名前ではなく、押した結果で決める。** 同じ部品でも置かれた文脈で区分が変わる — ラジオは、オーバーレイの中で選んだ瞬間に確定して閉じる場合 (`OptionRow`) は **A**、フォームの中で選ぶだけで確定しない場合 (`StorePicker`) は **C** である。各 Component の割当は [components.md](components.md) §1 共通事項の表が正
- [決定] **テキスト入力欄 (`Input`) は本節の対象外とする。** 押しても文字カーソルが入るだけで、押し間違いによる損失が無く、高さは文字サイズと上下余白で決まるためである。`Input` の高さ 48px はフォーム要素の見た目を揃えるための値であり、本節のタップ領域の規則ではない ([components.md](components.md) `Input`)
- [決定] **D 格子のセルは、高さの下限に加えてセル間に `{spacing.1}` (4px) 以上の隙間を設ける。** 密集した格子では、高さを満たしていても隣接セルとの境界で誤って押されるためである
- [事実] **指の側 (1024px 未満) で A と B が同値 (48px) なのは意図的である。** 指の側は身体の寸法が下限を決めるため、押し間違いの損失の差がほとんど効かない。ポインタの狙いが細かいマウスの側では損失の差がそのまま値の差になる
- [決定] **環境を分ける境界は `breakpoint.lg` (1024px) の 1 つのみとする。** §3.1 の文書レベルの見出しの段下げ、§7.1 の Modal の表示形態の切替 (1024px 未満 = `sheet` / 以上 = `popover`) と同じ境界に揃える。`pointer: coarse` 等のポインタ種別による分岐は採らない — 幅で切っている他の規則と境界が食い違い、実機でどちらが効いているかを切り分けられなくなるためである
- [事実] **1024px 以上の値はトークン化せず本表で示す。** 幅によって変わる値をトークンに持たせない扱いは §3.1 の見出しスケール・§4.1 の中央寄せの器の左右余白と同一である。semantic に置く 5 件のトークンはいずれも 1024px 未満の値を持つ
- [事実] 上表の 4 つの実寸 (48 / 40 / 32 / 24px) はいずれも既存 primitive `spacing` の段 (`spacing.12` / `.10` / `.8` / `.6`) であり、**新しい数値は追加していない**。Component は primitive を直接参照しないため、用途トークンとして semantic に置く。**44px 段 (`spacing` step 11) は本節でも追加しない**
- [事実] 本節は 2026-09-15 に、従前の**単一値 48px (`{size.tapTarget}`) を上表の 5 区分へ改めた** (Task 009-75・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §41)。従前の規定は Task 009-72 が本 DS の 44px / 48px の混在を 48px へ統一したものであり、混在の解消という目的は本節でも維持する (44px はどの区分でも用いない)。従前の単一トークン `{size.tapTarget}` は廃止した。**§2.3 品質下限・§8.3 の「タップ領域 48px」は、本節の区分のうち最も大きい値を示したものとして 48px のまま維持する** (下限そのものは区分ごとに本節の表が正)
- [事実] 未確定事項の一覧の「最小タップ領域 (44px) の spacing トークン」は Task 009-72 で解消済みであり、本節でもその状態を維持する (44px のトークンは追加しない)
- [事実] 本節は国内宿泊 (travel) `design.md` §4.2 の**定義体系を採用**したものである (Task 009-75・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §41)。**値は本 DS のファイルに独立して持ち、travel のファイルを参照・共有しない** (3 独立 DS の原則 = P1/ADR-0022)

## 5. 角丸・シャドウ・モーション

- 角丸: `sm` 4px / `md` 8px / `lg` 16px / `full` (pill)。用途トークンは **5 系統** (action / card / badge / overlay / field)
  - `radius.action` = full (pill) — **押す要素**。ボタン・CTA・チップ型操作要素・一覧の絞り込み/並び替えの起点 (`OverlayTrigger`)・`StickyBar` の条件ボタン。**値を受ける箱 (入力・選択系) は対象外**であり下記 `radius.field` を用いる (Task 009-76)。旧 R-01 (4px 固有形状を国内の意匠とする) は**廃止**した。縦グラデ・emboss 影・text-shadow も廃止 ([migration-map.md](migration-map.md))
  ...
  - `radius.field` = **md (8px)** — **値を受ける箱**。`Input` (テキスト/日付/数値)・`textarea`・`Select`・`SearchForm` のフィールドと、フォーム内の選択トリガー (店舗・日時等)。**押す要素とは形を分ける** (Task 009-76・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §42)
- [決定] **入力・選択系へ `radius.action` (pill) を使用しない** (Web部責任者判断 2026-09-16・Task 009-76・[Issue #196](https://github.com/tocoo/coocom-design-system/issues/196)・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §42)。pill は押す物のシグネチャであり、文字カーソルが入るだけの箱に与えると押せる要素に見える。値を受ける箱の角丸は `radius.field` (8px) に統一する
- [事実] 本節の従前の [決定] は次のとおりで、**線引きが逆であった**。①Task 009-63 (判断 F-5・[governance/owner-decisions.md](../../../governance/owner-decisions.md) §29) が `radius.input` (4px) を廃止して**入力要素を `radius.action` (pill) へ統一**した。②Task 009-72 ([同](../../../governance/owner-decisions.md) §38) が Select のみをその統一から外し `radius.select` (8px) とした。Task 009-76 はこの 2 件の線引きを改め、**揃える相手をボタンから「値を受ける箱どうし」へ変更**した。①が解消しようとした「同一フォーム内でボタンと入力欄が異なる形状を持つ」状態は、**ボタンと入力欄は異なる形状であってよい** (押す物と入れる箱の区別) という判断により解消の対象ではなくなる。`radius.input` (4px) の廃止は**維持**し、復活させない — 値を受ける箱の角丸は 8px であって 4px ではない。`radius.select` は `radius.field` へ統合して**廃止**した (値 8px・参照先 `radius.md` は不変・適用範囲のみ拡大)。用途トークンは **5 系統のまま**である
- [事実] 国内宿泊 (travel) の実装 (`tocoo/tocoo_travel` `origin/dev-ds` `eda3a549`) は、入力欄・テキストエリア・プルダウン・検索フォームの入力ユニット面のいずれにも pill を用いておらず 8px (一部 4px) である。pill (`--radius-action`) はボタン・チップ・一覧の絞り込み/並び替えの起点にのみ使われている (本 Repository で実測)。レンタカーの実装 (`tocoo/tocoo_rental_car`) は `epic-ds` を含め DS トークンを参照しているファイルが 0 件であり、本件に該当する実装は存在しない (同じく実測)
...
- [決定] **選ぶ操作の既定は「トリガー + オーバーレイ + 行」である** (Web部責任者判断 2026-09-16・Task 009-77・[Issue #198](https://github.com/tocoo/coocom-design-system/issues/198)・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §43)。`OverlayTrigger` を起点に `Modal` の `sheet` / `popover` (§7.1) を開き、`OptionRow` / `NavigationRow` の行で選ばせる。**ネイティブの `<select>` を用いる `Select` は例外**であり、①1 階層 ②件数が少ない ③選んだ値をその場に表示するだけで完結する ④選んだ後に他の入力と組み合わせない、の**すべてを満たす場合にのみ**用いる。判断に迷う場合は既定を採る。条件と実測の記録は [components.md](components.md) の `Select` 節が正
...
- [決定] **配置規則**:
  ...
  - `sheet`: 画面下端に貼り付き・全幅・上端 2 隅のみ角丸 (`radius.overlay` = 暫定 lg 16px)・ハンドル・行 48px・**高さは内容なり (最大 90vh)**
  - `popover`: 基準要素 (トリガー) を起点に配置し、下方に余地がなければ上方向へ反転する。**基準要素を必要とする** (drawer / sheet は不要)。**アンカー直下 8px・幅 240〜320px**
- [決定] **`sheet` の最大高は `90vh`・`popover` の幅は `240〜320px` とする** (判断 F-6・[governance/owner-decisions.md](../../../governance/owner-decisions.md) §29-3)。いずれも [components.md](components.md) に既にあった値をそのまま DS の規則へ格上げしたものであり、**新しい値を発明していない**。継承元の travel は同 2 項目を「依頼元の画面固有実装値 (85% / 420px / 680px) であり本 Repository で実査していない」として未確定に留めているが (travel `design.md` §7.1)、**rental-car は本節で確定する判断 (2026-09-07・Web部責任者) を採った**。3 独立 DS の原則 (P1/ADR-0022) により、本判断は travel・inbound の成果物へ及ばない。`90vh` は `drawer` の SP 上限と同値であり、**「選んで即閉じる」用途に対する上限**として置く (常用の高さではない)
...
- 最小タップ領域 (44px) の spacing トークン — `spacing` に 44px の段が存在せず (段は 0/4/8/12/16/20/24/32/40/48/64px)、44px は WCAG 2.2 の 2.5.5 Target Size (Enhanced) 相当で §2.3 が最低ラインとする AA (2.5.8 = 24×24 CSS px) からは導けない。**トークンを追加せず、未確定事項として残す** (品質下限としての 44px そのものは §2.3 で維持する)。**本項は Task 009-72 で解決した** — 最小タップ領域を **48px** に確定し、既存 primitive `spacing.12` を参照する用途トークン `size.tapTarget` を semantic に追加した (§4.2)。**Task 009-75 で単一値を 5 区分 × 幅 2 列へ改め**、トークンを `size.tapTarget.commit` / `open` / `toggle` / `cell` / `thumb` の 5 件へ分割した (§4.2)。**44px 段 (`spacing` step 11) は本 Task でも Task 009-75 でも追加していない**ため、上記の理由と「トークンを追加しない」判断は維持される
...
- [決定] 品質の下限 (適用外でも守る): コントラスト AA (支給画像内の文字を除く)・タップ領域 (**§4.2 の 5 区分 × 幅 2 列の表**が正。最大は 48px)・代替テキスト・色だけで伝えない
...
4. **押す要素 (ボタン・CTA・チップ・一覧の起点) は pill (`radius.action`)、値を受ける箱 (入力・選択系) は 8px (`radius.field`)**。4px の旧固有形状・グラデ・影を再現しない。非操作のラベルは `radius.badge` (4px) で、pill を使わない
4-1. 面 (背景) の上に文字を置くときは §2.4 の検証表で組み合わせを確認する。**表に無い組み合わせを検証済みとして扱わない**。リンクは §2.5 (文中は下線あり・hover で外さない・`opacity` を使わない)、未入力状態の文字色は §2.6、文書レベルの見出しは §3.1 (1024px 未満は `h1`〜`h4` の段を 1 つ下げる。`h5` / `h6` は 1rem のまま下げない) に従う
...
| Modal の form の a11y・実装 API 名 | ❓ 未定義 (§7.1)。a11y は DS 層で決定しない。最小タップ領域は Task 009-72 で **48px** (`size.tapTarget` = `{spacing.12}`) に確定し、**Task 009-75 で押した結果による 5 区分 × 幅 2 列へ改めた** (§4.2)。**44px 段 (`spacing` step 11) は追加しない** (従前どおり)。**`sheet` の最大高 (`90vh`) と `popover` の幅 (`240〜320px`) は ✅ 確定した** (2026-09-07・Web部責任者判断・判断 F-6・正本 = §7.1) | 実装実査 / 実装側タスク |
...
| E2 満車・受付終了 | CTA の非活性へ集約する方向のみ確定。**Button の disabled は Task 009-79 で確定した** (不透明度 `disabled.opacity` 0.5・色は変えない) ため前提は解消。残るのは**どの variant のボタンを非活性で出すか・文言・高さ・形** | オーナー判断 |
| 「選べない値」(カレンダーの過去日・満車のセル等) の表現 | ❓ 未判定 (Task 009-79 で起票)。Task 009-79 が確定したのは**操作要素が押せない状態** (`disabled`) の表現であり、選べない値の表現は対象外とした | オーナー判断 |
...
| 2026-09-07 | Task 009-63R2 の記述是正: PR [#171](https://github.com/tocoo/coocom-design-system/pull/171) コードレビュー 2 回目 ([issuecomment-5566325170](https://github.com/tocoo/coocom-design-system/pull/171#issuecomment-5566325170)) の指摘に対応。①§9 の 4-1 の見出し規則の要約が §3.1 の表と食い違っていたのを是正した — 「1024px 未満は段を 1 つ下げる」は `h1`〜`h4` に限られ、`h5` / `h6` は「同左 (段を下げない)」である。従前の要約に文字どおり従うと §3.1 が禁じる `typography.size.sm` (14px) を見出しに用いることになった。②§2.4 代替規則 1 の「通常テキスト基準 4.5:1 を満たす唯一の面」に**白文字に限る**限定を補った — 同節の検証表には濃色文字で 4.5:1 を満たす面が他に 6 組あり、同じリストの項 3 (accent 淡色面 5.84:1) とも矛盾していた。限定の内容は §2.4「A 割引率ラベルに限る白文字の例外」の記述と同一である。③§2.3 の `color.text.muted` の制限 (「装飾に限る」) に、§2.6 が同トークンを未入力状態の案内文字へ用いる**明示的な例外**である旨を追記した — 本 Task で §2.2 は追随済みだったが §2.3 のみ未追随で、品質下限の正本として読むと §2.6 が要求する用法を禁止する規則になっていた。④§2.4 の検証表に、[components.md](components.md) が通常の用法として規定しながら表に無かった 2 組を実測値とともに追加した (18 行 → **20 行**) — `color.action.primary.bg` `#2C50C8` × `color.action.primary.text` `#FFFFFF` **6.80:1** (Button `primary`)、`color.scheme.main.tint` `#E8EDFB` × `color.scheme.main.base` `#2C50C8` **5.81:1** (StepIndicator 完了・SecretPrice のマスク面の文字・Filter パネルの選択行)。いずれも AA 達成であり、是正の対象は「表に無い組み合わせを検証済みとして扱わない」と宣言する表の網羅性である。あわせて附属資料 [preview.rental-car.html](preview.rental-car.html) の収録行を「Component 9 種」から実体どおりの **13 種**へ是正した。**不変**: トークンの値・参照先・`$status`、§2.6 の決定内容、§3.1 の表そのもの、例外条項、Component の実装要件、inbound の成果物 | Claude Code |
| 2026-09-07 | Task 009-63R2 (追補): **`sheet` の最大高 (`90vh`) と `popover` の幅 (`240〜320px`) を確定**した (Web部責任者判断 2026-09-07・明示取得・判断 F-6・記録 = [governance/owner-decisions.md](../../../governance/owner-decisions.md) §29-3)。従前は §7.1「本節で定義しない事項」が両者を「本 Repository で実査していない。未確定として残す」としながら、[components.md](components.md) は同じ 2 値を `🚧` なしの確定値として記載しており、正本と Component 仕様が食い違っていた。**確定した値は `components.md` に既にあったものと同一**で、新しい値は発明していない。§7.1 の配置規則に両値を明記し、「本節で定義しない事項」から当該 1 行を削除し、未確定事項の一覧の該当行を更新した。**継承元の travel は同 2 項目を未確定のまま維持**しており (travel `design.md` §7.1・依頼元の画面固有実装値 85% / 420px / 680px は未実査)、3 独立 DS の原則 (P1/ADR-0022) により本判断は travel・inbound へ及ばない。**不変**: form の 3 値と切替規則・実装基盤 drawer 単一・backdrop の 3 形態共通・a11y と実装 API 名の未確定・トークンの値と `$status`。影響度 = **高** (判定者 = Web部責任者・判定日 2026-09-07・本件について明示取得。必要レビュー主体 = Web部責任者およびチーフデザイナー) | Claude Code |
| 2026-09-07 | Task 009-63R3 の記述是正: PR [#171](https://github.com/tocoo/coocom-design-system/pull/171) コードレビュー 3 回目 ([issuecomment-5567573901](https://github.com/tocoo/coocom-design-system/pull/171#issuecomment-5567573901)) の指摘に対応。①**§2.4 代替規則 1 と「A 割引率ラベルに限る白文字の例外」の「白文字が 4.5:1 を満たす面は 1 面のみ」を是正**した — Task 009-63R2 で同節の検証表へ追加した `color.action.primary.bg` `#2C50C8` × `color.action.primary.text` `#FFFFFF` **6.80:1** の行が、同じ節の当該記述の反証になっていた。両箇所を「**ラベル・非操作要素の面として使える範囲では**唯一の面」という限定へ改め、`action.primary` の面は Button `primary` の操作要素に固定された用途トークンでラベルの代替背景に用いない旨を明記した。②**§2.4 の検証表に、本書が実測値つきで規定しながら表に無かった白面上の 5 組を追加**した (20 行 → **25 行**) — `color.text.mutedStrong` `#616161` **6.19:1** (§2.3 の判読性を要する補助情報)、`color.text.muted` `#9E9E9E` **2.68:1** (§2.6 の未入力状態の案内文字・AA 未達を明示した例外)、`color.text.link` `#2C50C8` **6.80:1** / `linkHover` `#2340A6` **8.92:1** / `linkActive` `#1B3488` **11.09:1** (§2.5 の状態表)。いずれも本 Repository で WCAG 2.x 標準式により実測した値であり、**新しい色値・新しい規則は追加していない**。是正の対象は「表に無い組み合わせを検証済みとして扱わない」と宣言する表の網羅性で、従前は §9 の 4-1 に忠実に従うと §2.6 が必須とする「白面 + `color.text.muted`」が未検証扱いになった。③**2026-09-07 Task 009-63R2 の行の不変句を是正**した — 同句は「travel / inbound の成果物」を不変としていたが、同 Task で travel `design.md` §2.1 代替規則 1 の記述是正を行っており事実に反していた。**不変**: トークンの値・参照先・`$status`、既存 20 行の実測値、§2.4 の各 [決定] の内容 (白文字例外の適用範囲 = A 割引率ラベル + scheme 逆色面を含む)、§2.5 / §2.6 / §3.1 / §7.1 の決定内容、Component の実装要件、inbound の成果物。影響度は本 PR 全体と同じく**高** (§29-2。判定者 = Web部責任者・判定日 2026-09-07)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
| 2026-09-11 | Task 009-65: **`typography.fontWeight` のスケールを書体の配信段に揃えた** ([Issue #174](https://github.com/tocoo/coocom-design-system/issues/174)・Owner 判断 2026-09-11・記録 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §31)。①**primitive `typography.fontWeight` を 400 / 500 / 700 / 900 の 4 段から thin `100` / regular `400` / bold `700` / extraBold `800` の 4 段へ改めた** — `thin` (100) と `extraBold` (800) を**追加**し、`medium` (500) と `black` (900) を**削除**した。**根拠** = 書体 (`typography.fontFamily.heading` / `sans` / `latin` / `number` の先頭 = LINE Seed JP) が Google Fonts で配信するウェイトは **100 / 400 / 700 / 800 の 4 段のみ**であり (本 Repository で実測。`wght@100` / `@400` / `@700` / `@800` は `@font-face` が日本語 124 スライスぶん返り、`@200` / `@300` / `@500` / `@600` / `@900` は CSS ではなく `400: Font family not found` の HTML が返る)、**500 と 900 には対応する字面が存在しなかった**。削除した 2 段を `$value` で参照する semantic トークンは本サービスに存在しなかった (実測。`font.heading.weight` / `font.price.weight` / `label.weight.emphasis` は `bold`、`label.weight.neutral` は `regular` を参照)。②**§3 のウェイトの箇条を 4 段へ改め、[事実] 1 件を追加**した (配信段との一致・500 / 900 の削除根拠)。③**§3.1 の [決定]「新しいサイズ値・行間値・ウェイト値 (primitive) は追加しない」を改訂**した — サイズ値・行間値を追加しない方針は変更せず、ウェイト値については本 Task の追加・削除を [事実] として併記した。④**§2.1 の用語の定義「ウェイトの境界」を是正**した — 削除した `black` `900` を引いていた箇所を `extraBold` `800` へ、通常ウェイト側の例示を `thin` `100`・`regular` `400` へ改めた (**条件 (a) (b) (i) (ii) の定義・閾値・検証表の実測値はいずれも変更していない**)。⑤**`preview.rental-car.html`** の Google Fonts 読み込みを `wght@100;400;700;800` へ改め、生成器 `tools/gen-preview-tokens.py` を実行して CSS 変数とウェイト表を再生成した (`--check` が「一致」を返すことを確認)。**見出しの太さの既定は `font.heading.weight` (700) のまま不変**であり、**`thin` / `extraBold` を参照する semantic は設けていない** — 国内宿泊 (travel) `design.md` §3.1 が同 Task で定めた「太さ 100 を実寸 24px 以上の見出しに限り明示的に選択できる」規則は **travel に限り、本サービスへは自動適用しない** (3 独立 DS の原則 = P1/ADR-0022)。本サービスの見出しへ 100 を導入するか否かは別 Task で判断する。**不変**: §1・§2 の表・§2.1 の検証表と規則・§3 のサイズスケールと行高・§3.1 の表 (サイズ段・太さ 700・行間 1.3)・§4 以降、semantic の全トークン (値・参照先・`$status`)、`$meta.version`。影響度は Owner が判定 ([../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8)。新規 ADR・Decision ID は採番していない | Claude Code |
| 2026-09-15 | Task 009-72: **国内宿泊 (travel) の横断の規則と Select の定義体系を採用**した ([Issue #188](https://github.com/tocoo/coocom-design-system/issues/188)・Web部責任者判断 2026-09-15・明示取得・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §38)。①**§4.1 を新設**し、中央寄せの器の左右余白を幅ごとに 5 段 (〜639 = 16px `spacing.4` / 640〜767 = 24px `spacing.6` / 768〜1023 = 24px `spacing.6` / 1024〜1279 = 32px `spacing.8` / 1280〜 = 48px `spacing.12`) で定めた。幅ごとに値を持つ semantic トークンは新設せず表で示す。②**§4.2 を新設**し、指で操作する要素の最小の高さを **48px** (`{size.tapTarget}` = `{spacing.12}`) に確定した。**従前の 44px を改めた**もので、§2.3 品質下限・§8.3・未確定事項の一覧・[components.md](components.md) の 44px をすべて 48px へ揃えた (従前は Button の SP 全幅 CTA のみ 48px とするなど DS 内で 2 つの値が混在していた)。**44px 段 (`spacing` step 11) は追加していない**。③**§5 の角丸の用途トークンを 4 系統 → 5 系統**とし `radius.select` (= `{radius.md}` 8px) を追加した。`radius.action` (pill) が定める「入力要素」に対する **Select 限定の例外**であり、**判断 F-5 (`radius.input` の廃止・`Input` / `SearchForm` のフィールドの pill 統一) は維持する**。**不変**: §1・§2.1・§2.2・§2.4〜§2.6・§3・§6・§7・§9 の本文、§2.3 の他の下限 (WCAG 2.2 AA・代替テキスト・色だけで伝えない)、§4 の既存 4 行、§5 の既存 4 系統 (`radius.action` / `card` / `badge` / `overlay`) の値・参照先・`$status`、§7.1 の form 軸の定義と判断 F-6 の確定値、**未入力状態の文字色 (§2.6・判断 C-5)**、token の値・参照先・`$status` (追加 4 件を除く)、`$meta.version`・version、travel / inbound の成果物。**行っていないもの**: 44px 段の追加、`radius.input` の復活、幅ごとに値を持つ semantic トークンの新設、`Input` / `SearchForm` の角丸の変更、レンタカー独自 UI の定義、実装ファイルの変更。**採用したのは travel の定義体系であり、値は本 DS のファイルに独立して持つ** (travel のファイルを参照・共有しない = 3 独立 DS の原則 P1/ADR-0022)。影響度 = **高** (判定者 = Web部責任者・判定日 2026-09-15・本件について明示取得。必要レビュー主体 = Web部責任者およびチーフデザイナー = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10)。改訂着手の設計承認取得済み (§9・§20・§38)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
