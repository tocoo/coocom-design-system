...
- [事実] 面 (背景) として色を使う場合の許可される文字色・文字サイズ条件は面ごとに異なる。組み合わせ一覧・campaign accent 面の条件 (大きなテキスト相当のみ・DS 推奨最低値 20px / bold 以上)・20px 未満の代替規則・scheme inverse 面の文字色は `design.md` §2.1 が正。**`{color.text.onAccent}` の存在を理由にコントラスト確認を省略しない**
...
- `Card.slot.badge` (割引率ラベル・状態バッジ・カテゴリラベル等の非操作ラベル) の規則:
  ...
  - 面と文字色は文字サイズで分岐する
    - **20px / bold 以上** (大きなテキスト相当): 面 `{color.accent.campaign}` (#E4572E) + 文字 `{color.text.onAccent}` (≈3.68:1・大きなテキスト基準 3:1 のみ達成)
    - **20px 未満**: 面 `{color.accent.campaign}` を使用しない。既定は 面 `{color.surface.inverse}` (#212121) + 文字 `{color.text.inverse}` (≈16.10:1)。代替として accent を境界色・アイコン・点的装飾に限定する (詳細は `design.md` §2.1)
