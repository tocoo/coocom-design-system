...
| 取得した判断 ⓒ 依頼元が根拠とする 2026-07-31 の Owner 決定の扱い | 依頼元は「Owner 決定 (2026-07-31) により accent を採用」「Owner 例外承認」を根拠として挙げるが、`2026-07-30` / `2026-07-31` / `2026-08-` を含む記述は本 Repository 全体で **0 件**であり、当該 Owner 決定は本 Repository 内に記録が無かった。この事実を提示したうえで、Web部責任者は本記録 (2026-08-03 の現在判断 = ⓐ b2・ⓑ c1) をもって accent を面/点として用いる方向を**現在判断として記録**することを判断した。**過去 (2026-07-31) の判断主体・判断日は認定せず、本記録は現在判断として扱う** (§10/§11 と同じ区別)。commit 作成者・作成日・PR 作成者・merge 日を過去の判断主体・判断日として扱わない |
| 適用範囲 | **国内宿泊 (travel) に限定**。rental-car・inbound・3DS 横断へは自動適用しない |
...
| 依頼元の 2026-07-3x Owner 例外承認の扱い | 依頼元は E-5 について「Owner 例外承認」を根拠として挙げるが、`2026-07-30` / `2026-07-31` / `2026-08-` を含む記述は本 Repository 全体で **0 件**であり、当該例外承認は本 Repository 内に記録が無かった。本記録は過去の例外承認を認定せず、E-5 を現在判断 (Do の撤回・個数制約の撤廃) として扱う |
| 適用範囲 | **国内宿泊 (travel) に限定**。rental-car・inbound・3DS 横断へは自動適用しない |
