...
- [事実] 非構築: 会員ランク色 (**会員の階級表現**・破棄済)。**会員種別 (無料 / 有料) の 2 値の用途色 (`color.membership.free` / `color.membership.paid`) は本方針の対象外**であり、ラベル・タグ定義シート B 会員ロイヤリティ (Owner 決定 2026-08-07・依頼書 §5) の実装として追加済み — 破棄記録の適用範囲を階級表現に限る形で差し戻した (正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24・`semantic.travel.json` `$meta.notes`)

## 2. カラー

正: `semantic.travel.json` (用途) → `primitive.travel.json` (値)。以下は要約。配色は **共有ファウンデーション＋ブランド2スキーム** の二層 (TVL-0010)。全スキーム共通 (共有) は器/ニュートラル (白基調 warm-gray)・テキスト・境界のみ (surface/text/border)。状態色 (success/error)・特集アクセント・評価色 (星)・リンクは **per-scheme** = 各スキームが自スキームの値を保持 (TVL-0011。正式採用時に共通化): `color.scheme.main` = メイン(royal) / `color.scheme.sub` = サブ(indigo)。各スキームは tint(50)/soft(300)/base(500)/hover(600)/pressed(700)/ink(900)/**inverse(逆色)**/error/accent、および accent の 2 段 accentTint(100)/accentInk(800) — 計 11 段 (accent 2 段はラベル・タグ定義シート B の実装で追加・🚧 濃色段は仮色・§2.1 の b2)。
...
| `color.scheme.*.accentTint` / `accentInk` (accent 2 段・b2) | orange.100 / orange.800 (副色=coral.100 / coral.800) | `#F9CDBC` / 🚧 `#8A2E11` (副色 🚧 `#F8CBC2` / 🚧 `#8A2C18`) | 🚧 placeholder (淡色段の面は bound・濃色段が仮色・§2.1) |
| `color.accent.campaignTint` / `campaignInk` | scheme.main.accentTint / accentInk (副色=sub.*) | `#F9CDBC` / 🚧 `#8A2E11` | 🚧 placeholder (小サイズ 12px の非操作ラベル用・§2.1 の b2) |
| `color.label.discount.surface` / `text` (A 割引率) | scheme.main.inverse / text.inverse | `#C8912C` / `#FFFFFF` | bound (2.78:1・§2.1「A 割引率ラベルに限る白文字の例外」・AA 未達明示) |
| `color.label.category.surface` / `text` / `icon` / `border` (C カテゴリ・特集) | surface.default / text.strong / accent.campaign / border.default | `#FFFFFF` / `#212121` / `#E4572E` / `#CCCCCC` | bound (白面 × 濃色文字 ≈16.10:1・accent は**アイコン (非テキスト 3:1)** に限る ≈3.68:1・枠線は**白系の面の上に置く場合のみ** 1px・白面上 ≈1.61:1 で非テキスト 3:1 未達を明示・§2.1) |
| `color.label.stock` (E1 残室僅少) | state.error | `#D23A3A` | bound (白背景 ≈4.8:1・面なしの文字・写真上に出さない) |
| `color.membership.paid.surface` / `text` (有料会員) | accent.campaignTint / campaignInk | `#F9CDBC` / 🚧 `#8A2E11` | 🚧 placeholder (≈5.8:1・濃色段が仮色) |
| `color.membership.free.surface` / `text` (無料会員) | scheme.main.tint / ink | `#E8EDFB` / `#14224A` | bound (≈13.2:1) |
| `color.tag.neutral.surface` / `text` (D 施設属性) | surface.muted / text.body | `#F5F5F5` / `#424242` | bound (中立面・400・色を持たせず区分は順序で読ませる) |
...
| `color.accent.campaignTint` `#F9CDBC` (accent 淡色段・面) | `color.accent.campaignInk` `#8A2E11` (🚧 仮色) | ≈5.8:1 (概算・🚧) | 達成 (概算・🚧) | 達成 | **条件付き** (b2 = 小サイズ非操作ラベルの面 + 濃色文字。濃色段が **🚧 仮色**のため、実色値確定後に本 Repository で検証・確定する) |
| `color.accent.campaignTint` `#F9CDBC` (accent 淡色段・面) | `color.text.strong` `#212121` | ≈11.1:1 | 達成 | 達成 | 許可 (淡色面 + 既存の濃色文字。面の値は Owner 選定で確定・文字は既存 bound。b2 の代替として成立) |

- [事実] 淡色面 (`color.accent.campaignTint` `#F9CDBC`) は accent 淡色段 (b2) の面である。面の値は Owner 選定で確定 (bound) だが、面上の文字を accent 濃色段 (`color.accent.campaignInk` `#8A2E11`) とする組み合わせは**濃色段が 🚧 仮色**のため、上表の ≈5.8:1 は概算であり実色値確定後に本 Repository で検証・確定する (依頼 §9・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §14・§24)。`color.text.strong` を文字とする組み合わせ (≈11.1:1) は両値とも確定済みで検証できる。**表に無い組み合わせ・🚧 の行を確定済みとして扱わない**原則は維持する
- [事実] 上表は**文字と面の**組み合わせを対象とする。C カテゴリ・特集ラベルの枠線 (`color.label.category.border` = `color.border.default` `#CCCCCC`) は非テキストの境界であり本表の対象外だが、実測値を記録する — 白面 (`#FFFFFF`) 上 **1.61:1**・`color.surface.subtle` (`#F9F9F9`) 上 **1.53:1** (本 Repository で WCAG 2.x 標準式により実測)。いずれも**非テキスト UI 要素の 3:1 に達しない**。枠線は、白系の面の上では面のみで境界が生じないこと (白 × 白 1.00:1・白 × `surface.subtle` 1.05:1・同実測) に対する分離の補助であり、ラベルの識別は文字 (≈16.10:1) が担う。適合宣言は行わない (Task 009-60R・Web部責任者判断 2026-09-05・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27-3 ⓕ)。既存の border 3 段はいずれも白面上 3:1 に達しない (`subtle` `#e0e0e0` 1.32:1 / `default` `#cccccc` 1.61:1 / `strong` `#bcbcbc` 1.90:1 = 同実測)
...
2. accent を**非テキスト要素**に限定する — 境界色・アイコン・点的装飾 (非テキスト UI 要素の 3:1 は満たす)。**accent を文字色として明色面に置く方法は、この場合の代替にならない** (`default` 3.68:1 / `subtle` 3.50:1 / `muted` 3.38:1 はいずれも通常テキスト 4.5:1 に達しない。**accent 淡色面 (`color.accent.campaignTint` `#F9CDBC`) 上ではさらに下がり ≈2.5:1 となる** (依頼元観測は約 3.2:1 だが本 Repository の実測は ≈2.5:1) — 淡色面は白面から離れる分だけ分母が増える。これは accent を**文字色**として淡色面に置く場合であり、b2 = 淡色面 + accent **濃色段**文字 (別トークン `campaignInk`) とは区別する)
...
- [事実] 依頼元 (2026-08-03) の依頼 B に対し、Web部責任者は小サイズ (例 12px) のバッジについて **accent 淡色面 (淡色面 + 濃色文字) を新設する方向 (b2)** を選択した (正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §13)。**本方向はその後実装した** — ラベル・タグ定義シート B 会員ロイヤリティ (Owner 決定 2026-08-07) の実装として、primitive に accent の淡色段・濃色段 (`color.palette.orange.100` / `orange.800`、副色は `coral.100` / `coral.800`)、semantic に `color.scheme.*.accentTint` / `accentInk` と `color.accent.campaignTint` / `campaignInk` を追加した (Task 009-49・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。**淡色段の面の値は `#F9CDBC` (primitive `orange.100`) で確定 (bound・Owner 選定 2026-08-07)** だが、濃色段 (`orange.800` `#8A2E11`) と副色 coral の 2 段は **🚧 仮色**であり (このため semantic の `accentTint` / `campaignTint` も `$status` は placeholder)、淡色面 + 濃色文字の 4.5:1 成立は実色値確定後に本 Repository で検証する。**濃色段の実色値が確定するまでは** (i) に該当する 12px のバッジに既定 (neutral dark 面 `color.surface.inverse` + `color.text.inverse`) を用い、campaign accent を面として使用しない (`components.md` `Card.slot.badge` の (i) (ii) 代替 2 択と同じ扱い)
...
上記の「campaign accent 面上の文字」「scheme inverse 色を面として使用する場合」は、いずれも白文字を通常利用として許可しない (campaign accent 3.68:1 は (a) (b) のみ・scheme 逆色 2.78:1 / 2.15:1 は禁止)。このうち **scheme inverse 面**についてのみ、次の限定条件を**すべて**満たす場合に限り、白文字を**例外**として許容する (AA 未達を明示する)。**campaign accent 面は本例外の対象ではない** (Task 009-60 で縮小。下記 [事実])。

- [決定] **用途 = A 割引率ラベル (`color.label.discount`)** に限る。非操作の点的ラベルであっても、A 割引率ラベル以外 (B 会員種別・**C カテゴリ・特集**・D 施設属性・E 在庫/販売状態) は本例外の対象外とする。操作可能なバッジは badge ではなく action 系 Component として扱い、同じく対象外とする
...
- [決定] 本例外は現行の「`color.scheme.*.inverse` を面として使用する場合の文字色は `color.text.strong` に固定・白文字を使用しない」原則に対する**限定的な例外**であり、原則そのものは維持する (例外を A 割引率ラベル + 逆色面を超えて拡大しない)。**campaign accent 面の (i) (ii) 白文字禁止は例外なく維持する** — (i) (ii) に該当する文字へ campaign accent を面として使用しない規則 (上記「campaign accent を面として使用できない場合の代替規則」) と、`components.md` `Card.slot.badge` の (i) (ii) 代替 2 択 (① neutral dark 面 / ② accent 淡色面) はいずれも例外の対象外である
- [事実] 本条項の当初版 (Task 009-48・2026-08-10) は用途に「カテゴリラベル等」を、面に campaign accent を含めていた。C カテゴリ・特集ラベルの用途色を白面 (`color.label.category`) として確定した Task 009-60 (Web部責任者判断 2026-09-05・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27) により、campaign accent 面 + 白文字を用いるラベルは存在しなくなったため、上記のとおり適用範囲を A 割引率ラベル + 逆色面へ**縮小**した。縮小前の記述は `components.md` `Card.slot.badge` の「(i) (ii) では campaign accent 面を使用しない・代替は 2 択」と読みが割れる状態にあった (方向を記録した §20 本体は不変であり、本縮小は §27 の現在判断による)。**あわせて本条項の見出しを「非操作の点的ラベルに限る白文字の例外」から「A 割引率ラベルに限る白文字の例外」へ改称した** (Task 009-60R) — 縮小後は用途が A 割引率ラベルに限られ、旧見出しは本文の [決定] と逆の適用範囲を示していたため。旧見出し名は 2026-09-05 以前の変更履歴行に歴史記録として残る
- [事実] 本例外は割引率ラベル (A・`color.label.discount` = 逆色面 `#C8912C` + 白文字 2.78:1) を成立させるためのもの。方向は Task 009-44 (Web部責任者判断 2026-08-04・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §20) で確定し、ラベル・タグ定義シート A (画面設計 Owner 決定 2026-08-07) の実装として本条項を追加した (改訂着手の設計承認・影響度 = 同 §24)
- [事実] 同一の小サイズ・逆色面問題に対する別経路として、b2 (accent 淡色面 + 濃色文字 = `color.accent.campaignTint` / `campaignInk`) が併存する (§13・上記代替規則・検証表)。b2 は accent 濃色段が 🚧 仮色のため実色値確定後に検証する。本例外 (白文字) と b2 (淡色面) のどちらを最終解とするかは Owner 判断事項として残す
- [決定] 禁止: 本例外を非操作の点的ラベル以外 (本文・操作要素・大面積) へ適用する / 指定面以外へ拡大する / 縁取り・影でコントラスト不足を解決したものとして扱う / AA 未達を明示せずに用いる

#### 評価色と販促面色の用途境界

- [事実] `{color.scheme.main.inverse}` / `{color.scheme.sub.inverse}` を `$value` で参照する semantic トークンは **`color.icon.rating` (ReviewStars 星の色) と `color.label.discount.surface` (A 割引率ラベルの面・ラベル・タグ定義シート A の実装として追加 = `2d60656`・2026-08-10) の 2 件**である (`semantic.travel.json` 全文検索で確認・2026-09-01 時点)。**両者は値を共有するが用途は分離する** (下記 [決定] のとおり `color.icon.rating` トークンを面へ再束縛しない)。`color.brand.*` / `color.state.*` / `color.accent.campaign` はいずれも scheme の `base` / `error` / `accent` を参照し、`inverse` は参照していない。`color.scheme.sub.inverse` を参照する semantic トークンは存在しない (副色文脈での適用は per-scheme の記述として保持されている)
...
| 見出し (文書レベル h1〜h6・既定。役割による書体の選択は §3.1) | LINE Seed JP 700 / h1 2.5rem〜h6 1rem / lh 1.3 | **bound (§3.1)** |
| 見出し (一覧・カード) | LINE Seed JP 700 / h2 2rem (= 文書レベル h2 と同値・同一トークン) | **bound (Q2)** |
| Display・Hero・特集・詳細施設名 (書体選択の規則 = §3.1。「500–600」は書体の用途に関する記述であり、文書レベルの見出し `h1`〜`h6` の太さの既定は §3.1 の 700) | Noto Serif JP (明朝) 500–600 | **bound (Q8)** |
...
- [決定] **本節が定めるのは既定値であり、要素 (`h1`〜`h6`) と書体の対応を固定しない。** 書体の選択は**役割** (表現 = Display / 機能 = UI・文書構造。§3 の 2 書体構成) によって決まり、要素名によっては決まらない。したがって文書レベルの見出しの役割が §3 の Display・Hero・特集・詳細施設名にあたる場合は、その要素が `h1` であっても `font.display.family` (明朝) を**明示的に選択してよい**。既定 (`font.heading.family`) は、役割による明示的な選択がない場合に適用される値である
- [決定] 上記の明示的な選択の結果、サイズ・太さ・行間が本節の既定から外れることは**原則として認めない**。認めるのは、**利用目的とデザイン上の必然性が明確に示され、承認を得た場合**に限る (**承認主体 = Web部責任者**)。§8.3 が定める**適用外 (クリエイティブ)** にあたるもの (PR 帯・特集帯・企画のキービジュアル・支給バナー・テーマタイル) や LP はこれにあたる**例**であり、許容される場面をこの 2 つに限定するものではない。§8.3 の適用外にあたる場合は、あわせて同節の扱い (必然性・デザイン上の制約・目的の提示／**カラーは厳守**／品質の下限／どちらの段かの事前の宣言・宣言がなければ適用とみなす) に従う
- [決定] **サイズ・太さ・行間の既定は書体の選択と独立に定める。** 書体として明朝 (`font.display.family`) を明示的に選択しても、本節の既定 (`h1` 2.5rem 〜 `h6` 1rem・太さ 700・行間 1.3) はそのまま適用される。§3 の表が Display・Hero・特集・詳細施設名に記す「500–600」は書体の用途に関する記述であり、文書レベルの見出しの太さの既定を置き換えない。既定から外れる場合の扱いは上記の [決定] (原則として認めない・利用目的とデザイン上の必然性の提示および承認を得た場合に限る) による
- [事実] 本節が定めるのは書体の**選択規則**であり、個々の見出しが Display・Hero・特集・詳細施設名にあたるか否かの**該当判断は本書では行わない** (本節冒頭のとおり、画面別の情報構造は Screen Requirements 側の課題)。明示的に選択している箇所の棚卸しは未確定事項の一覧に起票している
...
- 角丸: `radius.sm 4 / md 8 / lg 16 / full 9999px` (bound)。用途トークンは 4 系統 (action / card / badge / overlay)
  ...
  - `radius.badge` = **確定 (bound)** sm (4px) — **非操作**のバッジ・ラベル (割引率ラベル・状態バッジ・カテゴリラベル・短い補助ラベル)。実px 4px を画面設計側が確定 (Owner 決定 2026-08-07) し placeholder → bound へ昇格済み (Task 009-56・値は sm = 4px のまま不変)
...
- [事実] `radius.badge` の参照先 sm (4px) は既存 primitive radius scale からの選定であり、新しい数値は追加していない。選定根拠は ①`radius.action` (pill) との形状差別化 ②`radius.card` (暫定 md) との階層差 ③寸法の小さいラベルでも形状が安定する。**実確定済み** — 画面設計側が同じ 4px を確定 (Owner 決定 2026-08-07) したため §14 の確認方法を満たし bound へ昇格した (Task 009-56・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。全ラベル (A〜G) の角丸は本トークンに統一する (器 `label.radius` が本トークンを参照)
- シャドウ: 実値未抽出 🚧 (follow-up #13。暫定3段で据置き = TVL-0008)
- [事実] 上記 placeholder (`radius.card`・`shadow.*`・`motion.*`) の**確認方法と個別確認主体**は Web部責任者判断 2026-08-03 で定義済み (確認方法 = 依頼元 (画面設計) の提出値を受領して確定・個別確認主体 = 作業担当者が照合し Web部責任者が確認。正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §14)。ただし本行の 3 件については実査 (提出値の受領) が未実施であり placeholder を維持する (bound 昇格・暫定値の確定は行わない)。placeholder 11 件 (semantic 4・primitive 7) の確定リストと追跡先区分 (`follow-up #13` = shadow / `follow-up #3` = motion / follow-up 番号なし = radius.card・radius.badge) は owner-decisions.md §14 (**同リストは記録時点 (`main` `cbc19c6`) のものであり、`radius.badge` はその後 Task 009-56 で bound へ昇格した**)。**`radius.overlay` (Task 009-36 で追加・`$status: placeholder`) は同リストに含まれておらず、確認方法・個別確認主体は §14 で定義されていない** (本行の 3 件とは扱いが異なる)
...
- [決定] 原則として使用しない表記: `-NN%` (マイナス付与)・`20％引き` (全角パーセント)・`▲20%`。同一の意味へ複数表記を混在させない (CTP-005 Consistent Terminology)
- [決定] 「最大NN%OFF」等の条件付き表現は、対象範囲 (対象プラン等) における最大値であり、かつ文脈で「最大」と判別可能な場合にのみ使用する (CTP-004 Evidence-backed Claims)。無条件の既定使用はしない
- [事実] 本表記規則は Task 009-45 (Web部責任者判断 2026-08-04・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §21) で確定した方向 (`-NN%` → `NN%OFF`・マイナス付与規則の撤回) を、ラベル・タグ定義シート A 割引率 (画面設計 Owner 決定 2026-08-07) の実装として反映したもの (改訂着手の設計承認・影響度 = 同 [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。旧基本形 `-NN%` と旧「符号」節 (マイナス付与) は撤回した
- [事実] 上記と異なる明示的な規則が Owner 決定またはブランドガイドラインに新たに存在する場合はそれを優先し根拠を記録する (上書き機構)

#### 数字の強調

- [決定] `NN%OFF` の**数字部分を `%OFF` 部分より視覚的に強調する**。強調は**サイズ**で行い、**数字部分は `%OFF` 部分より §3 の既存タイポグラフィスケールで 1 段上のサイズ段を用いる** (例: `%OFF` が `xs` 12px なら数字は `sm` 14px、`%OFF` が `sm` 14px なら数字は `md` 16px)。**新しいサイズ値 (primitive) は追加しない** (既存スケールの段の割当で表す)
- [決定] 書体は数字・`%OFF` とも `font.price` (`LINE Seed JP` `700` + tabular-nums) を用いる。ウェイトによる強調は行わず、**サイズ段の差のみ**で強調する (数字強調の比率 = 数字は `%OFF` の 1 段上。ラベル・タグ定義シート A・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24 の §24-3「数字を 1 段大きく表示」)
- [事実] 実サイズ段の割当 (器の 2 サイズ = sm: `%OFF` 12px / 数字 14px、md: `%OFF` 14px / 数字 16px)・箱の高さ・余白は semantic 器トークン (`label.fontSize.*` / `label.numberSize.*` / `label.height.*` / `label.paddingInline.*`) および `components.md` `Card.slot.badge` が正
- [決定] 旧「符号」節 (割引率であることを視覚的に示すため数値の前に半角マイナス `-` を付す) は**撤回した**。マイナス記号は付さない (Task 009-45 §21・ラベル・タグ定義シート A・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)
...
予約条件 (決済手段・返金可否・ポイント付与) の表示規則。**色値もトークンも新設せず、既存色の割り当て規則のみ**を定める。適用は予約条件 (F) に限る (ラベル・タグ定義シート F・Owner 承認 2026-08-10・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。

1. [決定] 予約条件は**面を持たないテキストの行**で示し、意味ごとのスロットに固める (キャンセル条件 → 支払い → 特典)。バッジの器 (面あり) には乗せない
2. [決定] ラベルの文字色を **4 役**に割り当てる (新しい割り当ては「得」の 1 行のみで、既存トークンの定義は変えない):
   - **得** (全額返金可・ポイント付与等) = `color.scheme.main.base` (暫定・現行踏襲・G-1)
   - **損** (返金不可・残室僅少等) = `color.state.error`
   - **中立** (決済手段等) = `color.text.mutedStrong`
   - **操作** (キャンセル規定を見る等) = `color.text.link`
3. [決定] **色だけで伝えず、文言を必ず併記する** (R9 / WCAG 2.2 AA)
4. [決定] アイコンは意味ごとに固定する (現地決済 = `yen-sign` / カード決済 = `credit-card` / 全額返金可 = `circle-check` / 返金不可 = `circle-xmark` / ポイント付与 = `coins`)。**アイコンに意味色は当てない** (アイコンの色は文字色に従う)
5. [決定] 並びを固定する: **返金 → カード決済 → 現地決済 → ポイント**
6. [決定] この行にプランの前提 (食事・泊数) や企画ラベル (C) を混ぜない
7. [決定] `color.state.error` の用途に「**利用者に不利な事実の明示**」を含める (E1 残室のひっ迫と共通・上流 CTP-010・`color.label.stock` と同じ用途拡張)
...
- [事実] 「得」= `color.scheme.main.base` は `color.text.link` (= 主色) と同値 (`#2C50C8`) であり、**リンクとの見分けは未解決** (暫定 G-1・依頼 §10)。見分けの手だて (非リンク文脈・下線の有無・アイコン併用等) は Owner / 設計判断事項として残す (未確定事項の一覧)
- [事実] 本規則は既存トークン (`scheme.main.base` / `state.error` / `text.mutedStrong` / `text.link`) の範囲で成立し、新色値・新トークンを伴わない。E 在庫 (残室 N 室 = 不利な事実 = error) にも同じ判定が当てはまる (色を使ってよい場面の定めを 1 つ置く形)

### 8.3 クリエイティブへの DS 適用範囲

PR 帯・特集帯・支給バナー等のクリエイティブに対する DS の適用範囲を **2 段**に分ける (ラベル・タグ定義シート H・Owner 承認 2026-08-10・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。

- [決定] **適用** (トークンのみ・逸脱なし): UI 部品・カード・ラベル全般 (A〜G)。ヒーローと宿カードの割引率もこちら (A の決定がそのまま適用)
- [決定] **適用外** (クリエイティブ): PR 帯・特集帯・企画のキービジュアル・支給バナー・テーマタイル。DS を**ガイドライン**として使う
- [決定] 適用外での扱い: 書体・文字サイズ・ウェイト・器の形 (角丸・余白・マージン)・傾きは、**必然性・デザイン上の制約・目的を示せば外せる** (規則ではなくガイドライン)。例: 企画の世界観上の必然性があれば `Futura` 等の他フェイスも可。ただし「なんとなく」では外さない
- [決定] **カラーは厳守**: 基調となるカラーは例外なく守る (必然性を示しても外せない)。文字色・面色は基調カラーの範囲内で選ぶ
- [決定] 品質の下限 (適用外でも守る): コントラスト AA (支給画像内の文字を除く)・タップ領域 `44px`・代替テキスト・色だけで伝えない (R9 / WCAG 2.2 AA)
- [決定] **グラデーションは審査対象**とする (一律禁止でも一律許可でもなく場合による)。使う側が用途と範囲を出して判断を仰ぐ。**適用側 (UI) に対しては現行の「グラデーションなし」がそのまま残る**
- [決定] 運用: 新しい帯やバナーを作るときに**どちらの段かを先に宣言する**。宣言がなければ適用とみなす。適用外を選んだ場合はガイドラインを外れる項目ごとに理由を添える
- [事実] グラデーションの**審査基準の策定と審査主体の設置**は運用体制の問題として DS 側に委ねられた新規論点である (画面設計側の決定は「審査対象とする」まで)。審査基準・審査主体の実策定は本節に含めず別途 (Task 009-55・Issue [#147](https://github.com/tocoo/coocom-design-system/issues/147))
- [事実] 本節は非 UI クリエイティブへの適用範囲の例外を Governance 横断ルールとして新設する方向 (§23・Task 009-47) と同軸である。§23 の「カラーもトークン指定外を許容」(ⓑ) と本節の「カラーは厳守」の差分、および原則正本の置き場所 (Governance 横断 vs 本節) は §23 の設計承認プロセス・Task 009-55 で扱う (本節は travel `design.md` への明文化に留める)
...
| — | 割引率の表記の基本形 (§8.1) — `-NN%` から `NN%OFF` への変更 | ✅ **実装済み** (Task 009-51・ラベル・タグ定義シート A・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。§8.1 の基本形を `-NN%` → **`NN%OFF`** へ改訂し、旧「符号」節 (マイナス付与規則) を撤回。数字強調 (数字は `%OFF` より 1 段上のサイズ段) を「数字の強調」節に追加。全角 `％`・`▲NN%` 禁止維持・半角 `%`。「最大NN%OFF」の使用条件を明記。`components.md` `Card.slot.badge` の §8.1 参照も `NN%OFF` へ整合 (PR-3) |
...
| — | campaign accent 面上で §2.1 の (a) (b) を満たさない文字 ((i) 20px 未満のすべて／(ii) 20px 以上 24px 未満かつ通常ウェイト) を成立させる濃色背景の追加要否 | 🚧 b2 = accent 淡色面を**追加済み・実色値確定待ち** (Task 009-49・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §13・§24)。`Card.slot.badge` の (i)(ii) 代替を 2 択化 (neutral dark 面 / accent 淡色面 + 濃色文字)。primitive/semantic に accent 淡色段 (`orange.100` `#F9CDBC` bound) / 濃色段 (`orange.800` 🚧 仮色) と `campaignTint`/`campaignInk` を追加。濃色段が 🚧 仮色のため淡色面 + 濃色文字の 4.5:1 成立は実色値確定後に検証。白文字例外 (§20) と併存し、どちらを最終解とするかは Owner 判断事項 |
| — | バッジ/ラベルの白文字非準拠運用 (割引率 `#C8912C` 面/白 14px・小サイズ campaign accent 面/白 12px) の扱い | ✅ **実規則化済み** (Task 009-48・ラベル・タグ定義シート A・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。§2.1「A 割引率ラベルに限る白文字の例外」条項・`components.md` `Card.slot.badge` の例外条項として追加。**非操作の点的ラベル + campaign accent/scheme 逆色面 に限り白文字を例外許容** (サイズ/ウェイト不問・AA 未達明示・`#C8912C`×白 2.78:1 等)。白文字が成立する面は `color.surface.inverse` (16.10:1) の 1 面のみ。原則 (白文字禁止) は維持し例外を指定面に限る。b2 (accent 淡色面) は別経路として併存・実色値待ち。**2026-09-05 に適用範囲を縮小** — Task 009-60 (Web部責任者判断・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27) で C カテゴリ・特集ラベルを白面へ確定したことに伴い、例外の用途を **A 割引率ラベルに限り**、面を **scheme 逆色に限る** (campaign accent 面 + 白文字は例外から外し、(i) (ii) の白文字禁止を例外なく維持) |
| — | `radius.badge` の実px | ✅ **確定 (bound)** (Task 009-56・4px = sm・Owner 選定 2026-08-07・§14 の確認を満たす・値は不変)。placeholder → bound へ昇格済み。全ラベル (A〜G) の角丸を本トークンに統一 (器 `label.radius`) |
| — | accent 淡色段/濃色段・サブスキーム (coral) 2 段の実色値 | 🚧 実査待ち (Task 009-56・依頼 §9・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §14・§24)。accent 淡色段 (`orange.100` `#F9CDBC`) は Owner 選定で **bound**。accent 濃色段 (`orange.800` `#8A2E11`) は **🚧 仮色** placeholder。サブスキーム `coral` 2 段 (`#f8cbc2` / `#8a2c18`) も **🚧 仮色** placeholder (メイン揃えの可否含め未実査)。淡色面 + 濃色文字の 4.5:1 成立検証は実色値確定後。仮色 3 値を差し替えれば b2・会員ロイヤリティ (B) の面/文字が追随する |
| — | C カテゴリ・特集ラベルの用途色 (`color.label.category`) | ✅ **確定** (Task 009-60・[Issue #162](https://github.com/tocoo/coocom-design-system/issues/162)・Web部責任者判断 2026-09-05・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27)。**accent の帰属** = accent を**面**として持つのは B 会員種別 (`color.membership.paid` = accent 淡色段) のみ、C カテゴリ・特集は accent を**アイコン (点)** として持つ、という用途分離で確定。C の用途色 `color.label.category` (`surface` = `color.surface.default` `#FFFFFF` / `text` = `color.text.strong` `#212121` = ≈16.10:1 / `icon` = `color.accent.campaign` `#E4572E` = 非テキスト 3:1) を `semantic.travel.json` に定義した (既存 semantic への参照のみ・新しい色値なし)。12px (`label.fontSize.sm`) でもサイズ・ウェイトの条件なく成立するため、C には (i) (ii) の代替 2 択 (① neutral dark 面 / ② accent 淡色面) を要しない。**枠線 `border` は Task 009-60R で追加**した (白系の面の上に置く場合のみ・本一覧の分離手段の行) |
| — | 白面の C カテゴリ・特集ラベルを白系の面 (`surface.default` / `subtle`) の上に置く場合の分離手段 | ✅ **確定** (Task 009-60R・Web部責任者判断 2026-09-05・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27-3 ⓕ)。**白系の面 (`color.surface.default` `#FFFFFF` / `color.surface.subtle` `#F9F9F9`) の上に置く場合にのみ、`color.label.category.border` (= `color.border.default` `#CCCCCC`) の枠線を `border.width.thin` (1px) で引く**。写真の上・有色面の上では枠線を引かず面のみで分離する (影・スクリムは用いない = ラベル・タグ定義シートと整合)。枠線は白面上 ≈1.61:1 で**非テキスト UI 要素の 3:1 に達しない**ことを明示する (§2.1 の [事実])。写真・グラデーションを面とする場合の scrim の要否は別論点 (依頼 E-1・本一覧の別行) |
| — | C カテゴリ・特集ラベルのアイコンの字形とサイズ | ❓ 未定義 (Task 009-60R で顕在化・字形は Task 009-61 = [Issue #166](https://github.com/tocoo/coocom-design-system/issues/166) で起票)。C の意匠にアイコン (`color.label.category.icon`) を含めたが、**字形 (どのアイコンを用いるか) とサイズのいずれも定めていない**。**字形**は §6 がアイコン体系を Font Awesome 6 と定めるのみで、個々のラベルへの割当規則を本書・`components.md` のいずれも持たない ([preview.travel.html](preview.travel.html) の見本の `fa-bolt` は**例示**である)。**サイズ**は本書のアイコン体系のサイズスケール `iconSize.sm〜xl` の最小段は **16px** で、ラベルの器 sm (高さ `label.height.sm` 20px・文字 `label.fontSize.sm` 12px) に対して当てる値が本書・`components.md` のいずれにも無い ([preview.travel.html](preview.travel.html) の見本は継承の 12px で描いており、これは**スケール外の例示**である)。rental-car の C1 / C2 ([../../rental-car/design-system/labels-tags.rental-car.md](../../rental-car/design-system/labels-tags.rental-car.md)) も同じく未規定。決定主体 = Web部責任者・チーフデザイナー |
| — | E2 販売終了 (直前割一覧・非活性 CTA) の意匠値 | 🚧 方向のみ (Task 009-53・依頼 §10)。CTA に集約する方向 (ラベルを増やさず非活性の「販売終了」ボタンへ) のみ確定。面色・文字色・高さ・形・文言は未確定。DS の Button 非活性状態 (disabled) の定義 (follow-up #4) が前提のため据え置き |
| — | F 予約条件「得」の文字色 (`scheme.main.base`) とリンク (`text.link`) の見分け | 🚧 未解決 (Task 009-54・§8.2・依頼 §10)。「得」= `scheme.main.base` は `text.link` と同値 (`#2C50C8`) で、押せないのにリンクに見える。メリット色は G-1 (現行 base) で暫定。見分けの手だて (非リンク文脈・下線・アイコン併用等) は Owner / 設計判断事項 |
| — | クリエイティブのグラデーション審査基準・審査主体 | 🚧 未策定 (Task 009-55・§8.3・依頼 §8)。グラデーションを「審査対象とする」までが画面設計側の決定。審査基準の策定と審査主体の設置は運用体制の問題として DS 側に委ねられた新規論点。§23 (Governance 横断) との原則正本の置き場所・「カラー厳守」vs §23ⓑ の差分も併せて Task 009-55 で扱う |
...
| — | 文書レベルの見出しに明朝 (`font.display.family`) を明示的に選択している箇所の棚卸しと、既定 (サイズ・太さ・行間) から外れている箇所の例外要件 (利用目的とデザイン上の必然性が示され、承認 = Web部責任者 を得ているか) の充足状況 | 🚧 実査待ち ([Issue #163](https://github.com/tocoo/coocom-design-system/issues/163))。本書は書体の選択規則のみを定め、個々の画面での該当判断・記載の有無は確定しない。実装 Repository (`tocoo/tocoo_travel`) 側の適用作業は本書の範囲外 |
...
| 2026-08-10 | Task 009-48/009-49/009-50/009-51/009-53/009-54/009-55/009-56 (ラベル・タグ定義シート A〜H の実装。design.md では白文字例外(009-48)/検証表(009-50)/§8.1(009-51)/F(009-54)/H(009-55)/error在庫(009-53)の実改訂に加え、b2(009-49)・実査値(009-56)の未確定事項行も更新): **§2.1** に「非操作の点的ラベルに限る白文字の例外」条項を新設 (非操作の点的ラベル + campaign accent/scheme 逆色面 に限り白文字を例外許容・サイズ/ウェイト不問・AA 未達明示・原則は維持) し、検証表に淡色面 2 行 (淡色面 × accent 濃色段 = 🚧 仮色・淡色面 × `text.strong` ≈11.1:1) を追加、代替規則 2 に「accent を文字色として淡色面に置くと ≈2.5:1 (依頼元観測 3.2:1)」の補記を追加。**§8.1** の基本形を `-NN%` → `NN%OFF` へ改訂し旧「符号」節 (マイナス付与) を撤回・「数字の強調」節を新設 (数字は %OFF より 1 段上のサイズ段)・「最大NN%OFF」の使用条件を明記。**§8.2 予約条件の表示規則を新設** (面なしテキスト行・文字色 4 役・アイコン固定・並び固定・文言併記・`state.error` の不利事実用途・得損の判定基準。色値/トークン新設なし)。**§8.3 クリエイティブへの DS 適用範囲を新設** (適用/適用外の 2 段・カラー厳守・品質下限・グラデーション審査対象・宣言運用)。**未確定事項の一覧**を更新 (割引率表記=✅実装済み・白文字非準拠=✅実規則化済み・`radius.badge`=✅bound・b2=仮色で追加済み) し 5 行追加 (accent/coral 実色値=🚧仮色・C 特集=据え置き・E2=方向のみ・F 得の見分け=未解決・グラデ審査基準/主体=未策定)。正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24。**不変**: §1・§3〜§7・§9、既存 token の値・参照先 (トークン追加は別 PR)、rental-car / inbound の成果物。C カテゴリ・特集の用途色・E2 の値は据え置き (依頼 §10)。影響度 = **高** (判定者 = Web部責任者・判定日 2026-08-10・§24)。改訂着手の設計承認取得済み (§9・§20・§24)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
| 2026-09-01 | **§5 の記述是正** (`radius.badge` の状態): Task 009-56 (`2d60656`・2026-08-10) で `semantic.travel.json` の `radius.badge` を placeholder → **bound** (4px = sm・Owner 決定 2026-08-07) へ昇格し、同 Task で「未確定事項の一覧」も ✅ 確定 (bound) へ更新したが、**§5 本文が Task 009-36 (`56abbf9`・2026-08-03) 時点の「暫定 sm (4px)・🚧 実px未取得」のまま残り**、同一ファイル内で §5 と未確定事項の一覧が矛盾していた。最新の状態 (bound) を正として §5 の 3 箇所を是正した — ①角丸の用途トークン一覧の `radius.badge` 行を「確定 (bound) sm (4px)・Owner 決定 2026-08-07 で確定・Task 009-56 で昇格済み・値は不変」へ ②`radius.badge` の選定根拠の [事実] 行の「暫定選定」「実確定は `radius.card` と同じ実査待ち区分」を「選定」「実確定済み (§14 の確認方法を満たし bound へ昇格・器 `label.radius` が本トークンを参照)」へ ③placeholder の確認方法を述べる [事実] 行の列挙から `radius.badge` を除き、owner-decisions.md §14 の placeholder 11 件リストが**記録時点 (`main` `cbc19c6`) のもの**であり `radius.badge` はその後 bound へ昇格した旨を明記。**不変**: `radius.badge` の値 (sm = 4px)・参照先・`$status` (`semantic.travel.json` 側は変更していない)・`radius.action` / `radius.card` / `radius.overlay` の記述・シャドウの記述・§5 以外の全節・未確定事項の一覧・`components.md` (既に bound と記載済み)・`governance/owner-decisions.md` §14 / §24 (記録時点の記述として保持)。**行っていないもの**: トークンの値・`$status` の変更、新規 ADR・Decision ID の採番、placeholder 件数の再集計 (§14 の記録そのものは改定しない)。影響度 = **低** (判定者 = Web部責任者、判定日 = 2026-09-01、本件について明示取得。必要レビュー主体 = Web部レビュー担当者 = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10。トークンの値・`$status` に及ばず、既に JSON 側で確定済みの状態を記述へ反映する是正と、非正本の付属資料の追加であるため) | Claude Code |
| 2026-09-01 | **§1・§5 の記述是正 2 件** (同種の反映漏れの継続点検で検出): ①**§1「非構築: 会員ランク色 (破棄済)」** — Task 009-52 (`2d60656`・2026-08-10) で `semantic.travel.json` に `color.membership.free` / `color.membership.paid` を追加し、`$meta.notes` では「破棄したのは階級 (ランク) 表現であり、無料 / 有料の 2 区分の会員種別色はこれに該当しない」として破棄記録の適用範囲を階級表現に限る形で差し戻した (正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24) が、**§1 は差し戻し前の記述のまま**で、要約だけを読むと会員種別色が本方針に反するトークンと読める状態だった。§1 に「会員ランク色 = 会員の階級表現」であること、会員種別 (無料 / 有料) の 2 値の用途色は本方針の対象外であり追加済みであることを明記した。②**§5 の placeholder 列挙** — `radius.overlay` (Task 009-36 で §5 に追加・`$status: placeholder`) が同節の placeholder 確認方法を述べる [事実] 行の列挙に含まれておらず、同一節内で「🚧 実px未取得」と「確認方法の対象 3 件」が食い違っていた。owner-decisions.md §14 の 11 件リスト (`main` `cbc19c6` 時点) にも `radius.overlay` は含まれないため、**同トークンは §14 で確認方法・個別確認主体が定義されていない**という事実を明記した (§14 の記録そのものは改定しない)。**不変**: トークンの値・参照先・`$status` (JSON 側は一切変更していない)・`color.membership.*` の内容・`radius.overlay` の値 (暫定 lg 16px)・§1 の他の [事実] 行・§5 の他の記述・§2〜§9 の他節・未確定事項の一覧・`components.md`・`governance/owner-decisions.md` §14 / §24。**行っていないもの**: §2 の用途 (Semantic) 要約表への新用途色 (`color.label.*` / `color.membership.*` / `color.tag.neutral` / `color.accent.campaignTint` / `campaignInk` / `color.scheme.*.accentTint` / `accentInk`) と器 `label.*` の行追加 (同表は「以下は要約」と明記しており矛盾ではないため、追加の要否は別途判断)、`radius.overlay` の確認方法の新規定義、placeholder 件数の再集計。影響度 = **低** (判定者 = Web部責任者、判定日 = 2026-09-01、本件について明示取得。必要レビュー主体 = Web部レビュー担当者 = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10。トークンの値・`$status` に及ばず、既に JSON 側で確定済みの状態を記述へ反映する是正と、非正本の付属資料の追加であるため) | Claude Code |
| 2026-09-01 | **§2.1 の記述是正 2 件** (同種の反映漏れの継続点検で検出。いずれも Task 009-48〜009-56 のバッチ (`2d60656`・2026-08-10) が JSON 側に加えた変更を §2.1 の [事実] 行へ反映できていなかったもの): ①**b2 (accent 淡色面) の [事実] 行** — 「accent 淡色段 (50 / 100 相当。現行 palette に不在) の実色値は未取得であり、本書では新規 primitive・semantic を追加していない」は Task 009-38 (2026-08-03) 時点の記述で、実際には primitive に `color.palette.orange.100` / `orange.800` (副色 `coral.100` / `coral.800`)、semantic に `color.scheme.*.accentTint` / `accentInk` と `color.accent.campaignTint` / `campaignInk` が**追加済み**であり、淡色段の面の値も `#F9CDBC` (primitive `orange.100`) で **bound** (Owner 選定 2026-08-07) である。追加済みであること、未取得なのは**濃色段 (`orange.800`) と副色 coral の 2 段**であること、semantic 側が placeholder である理由 (濃色段が 🚧 仮色) を明記し、「濃色段の実色値が確定するまでは (i) の 12px バッジに neutral dark 面を用いる」という暫定の扱いが `components.md` `Card.slot.badge` の (i) (ii) 代替 2 択と同じであることを補った。②**評価色と販促面色の用途境界の [事実] 行** — 「`{color.scheme.main.inverse}` / `{color.scheme.sub.inverse}` を `$value` で参照する semantic トークンは `color.icon.rating` の **1 件のみ**」は、同バッチで `color.label.discount.surface` (= `{color.scheme.main.inverse}`) が追加されたことにより事実と食い違っていた。全文検索の実測 (2026-09-01 時点) に基づき **2 件** (`color.icon.rating` / `color.label.discount.surface`) へ是正し、値を共有するが用途は分離する (同行の [決定] のとおり `color.icon.rating` を面へ再束縛しない) 旨を補った。**不変**: トークンの値・参照先・`$status` (JSON 側は一切変更していない)・§2.1 の検証表 (全 13 行のコントラスト比を WCAG 2.x 標準式で再計算し記載値と一致することを確認)・代替規則 1〜3 の本文・白文字の例外条項・`color.scheme.sub.inverse` を参照する semantic が存在しない事実・§2.1 以外の全節。**行っていないもの**: 用途境界の [決定] 行の変更、`color.icon.rating` / `color.label.discount` の定義変更、副色 (sub) 側への用途色の追加。影響度 = **低** (判定者 = Web部責任者、判定日 = 2026-09-01、本件について明示取得。必要レビュー主体 = Web部レビュー担当者 = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10。トークンの値・`$status` に及ばず、既に JSON 側で確定済みの状態を記述へ反映する是正と、非正本の付属資料の追加であるため) | Claude Code |
| 2026-09-01 | **§2 の用途 (Semantic) 要約表への反映 (7 行追加) と、スキーム段の記述是正**: Task 009-48〜009-56 のバッチ (`2d60656`・2026-08-10) で追加した用途色が §2 の要約表に 1 件も反映されておらず、同表を通読しても新用途色の存在に到達できない状態だった。**追加した 7 行** — `color.scheme.*.accentTint` / `accentInk` (accent 2 段・b2)、`color.accent.campaignTint` / `campaignInk`、`color.label.discount.surface` / `text` (A 割引率)、`color.label.stock` (E1 残室僅少)、`color.membership.paid.surface` / `text` (有料会員)、`color.membership.free.surface` / `text` (無料会員)、`color.tag.neutral.surface` / `text` (D 施設属性)。参照先・値・`$status` はいずれも JSON の実測値を転記し、コントラスト比は WCAG 2.x 標準式で再計算した値を記載する (白背景 × `#D23A3A` = 4.77:1 → 「≈4.8:1」/ `#E8EDFB` × `#14224A` = 13.21:1 → 「≈13.2:1」/ `#F9CDBC` × `#8A2E11` = 5.84:1 → 「≈5.8:1」/ `#C8912C` × 白 = 2.78:1)。**あわせて §2 冒頭の是正** — 「各スキームは tint(50)/soft(300)/base(500)/hover(600)/pressed(700)/ink(900)/inverse/error/accent」の 9 段の列挙は、同バッチで `accentTint` (100) / `accentInk` (800) を追加したことにより実態と食い違っていた (semantic の実測は 11 段)。11 段へ是正し、accent 2 段が 🚧 濃色段仮色であることと §2.1 の b2 を参照先として明記した。**不変**: トークンの値・参照先・`$status` (JSON 側は一切変更していない)・既存 26 行の内容・§2 の [事実] 3 行 (テキスト 4 段・用途別名・品質基準)・§2.1 の検証表と各 [決定]・§3 以降の全節。**行っていないもの**: 器 `label.*` (寸法トークン) の §2 への追加 (カラー節の対象外。正は `semantic.travel.json` と `components.md` `Card.slot.badge`)、副色 (sub) 側の用途色の新設、placeholder の bound 昇格。影響度 = **低** (判定者 = Web部責任者、判定日 = 2026-09-01、本件について明示取得。必要レビュー主体 = Web部レビュー担当者 = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10。トークンの値・`$status` に及ばず、既に JSON 側で確定済みの状態を記述へ反映する是正と、非正本の付属資料の追加であるため) | Claude Code |
| 2026-09-04 | Task 009-59: 文書レベルの見出しの**書体は要素ではなく役割で選ぶ**ことを明文化した ([Issue #163](https://github.com/tocoo/coocom-design-system/issues/163))。現行 §3.1 は明朝を「明示的に選択する書体」と述べる一方で、要素 (`h1`〜`h6`) と書体の対応を固定しないことが明文化されておらず、実装側で「`h1` に明朝を当てるのは §3.1 に反するか」という判断が画面ごとに発生していた。**§3.1 に [決定] 2 行・[事実] 1 行を追加** — ①本節が定めるのは既定値であり要素と書体の対応を固定しないこと、書体の選択は役割 (表現 = Display / 機能 = UI・文書構造) で決まり要素名では決まらないこと、役割が §3 の Display・Hero・特集・詳細施設名にあたる場合は `h1` であっても `font.display.family` (明朝) を明示的に選択してよいこと、既定 `font.heading.family` は明示的な選択がない場合に適用される値であること。②明示的な選択の結果サイズ・太さ・行間が既定から外れる場合は当該画面仕様または当該 Component 仕様へ明示記載すること (暗黙の上書きを認めない。Component について本節が既に定める扱いと同一の規則を文書レベルの見出しにも及ぼす)。③本節が定めるのは選択規則であり、個々の見出しが Display・Hero・特集・詳細施設名にあたるか否かの該当判断は本書では行わないこと。**§3 の表**の Display 行の用途欄に書体選択の規則の参照 (§3.1) を付した。**未確定事項の一覧に 1 行追加** (明朝を明示的に選択している箇所の棚卸しと、既定から外れる値の明示記載の有無 = 🚧 実査待ち)。**不変**: §3.1 の既定値そのもの (`h1` `4xl` 2.5rem 〜 `h6` `md` 1rem・太さ 700・行間 1.3・既定書体 = ゴシック)、§3 の「Noto Serif JP (明朝) 500–600」・サイズスケール・行間の各記述、`font.heading.*` / `font.display.*` の `$value`・参照先・`$status`・`$description`、`semantic.travel.json`・`primitive.travel.json`・`components.md`・`preview.travel.html`、§1〜§2.3・§4〜§9、version 表記、rental-car / inbound の成果物。**作成・追加していないもの**: 新規 primitive・新規 token、明朝を明示的に選択した場合のウェイトの規定 (Web部責任者判断 2026-09-04 により §3.1 には書かない。§3 の 500–600 の記述は不変であり、供給側の差異は実装 Repository 側で追跡)、画面別の見出し階層・semantic role の割当、実装側の適用作業。改訂着手の設計承認は [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §26 に記録 (承認主体 = Web部責任者・承認日 2026-09-04)。影響度は**低** (判定者 = Web部責任者・判定日 2026-09-04・本件について明示取得。必要レビュー主体 = Web部レビュー担当者 = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
| 2026-09-05 | Task 009-59R: Task 009-59 の記述是正 (PR #164 のコードレビュー指摘 1 に対応)。§3.1 に明朝を明示的に選択した `h1` の太さについて **§3.1 の既定 (700) と §3 の表の「500–600」のどちらが規範か**が読み取れない状態が残っていたため、**[決定] 1 行を追加** — サイズ・太さ・行間の既定は**書体の選択と独立**に定め、明朝 (`font.display.family`) を明示的に選択しても本節の既定 (`h1` 2.5rem 〜 `h6` 1rem・太さ 700・行間 1.3) がそのまま適用されること、§3 の表が Display・Hero・特集・詳細施設名に記す「500–600」は書体の**用途に関する記述**であって文書レベルの見出しの太さの既定を置き換えないこと、既定から外す場合の扱いは §3.1 の他の [決定] によること (本行の当初版は「当該画面仕様または当該 Component 仕様へ明示記載」と記していたが、同日の**指摘 4 対応**により後述のとおり書き換えたため、本記述もそれに合わせた)。**不変**: §3 の「Noto Serif JP (明朝) 500–600」の記述、§3.1 の既定値そのもの (太さ 700 を含む)、Task 009-59 で追加した [決定] 2 行・[事実] 1 行、§3 の表の他行、未確定事項の一覧、`font.heading.*` / `font.display.*` の `$value`・参照先・`$status`、`semantic.travel.json`・`primitive.travel.json`・`components.md`・`preview.travel.html`、§1〜§2.3・§4〜§9、rental-car / inbound の成果物。**作成・追加していないもの**: 新規 primitive・新規 token、明朝専用のウェイト規定 (本行は既定が書体に依存しないことを述べるものであり、明朝に固有のウェイトを新設しない)、実装側の適用作業。あわせて**指摘 2 に対応**し、§3 の表の「見出し (文書レベル h1〜h6)」行の用途欄に「**既定。役割による書体の選択は §3.1**」を付した (同表だけを読むと文書レベルの見出しが `LINE Seed JP` に固定されていると読め、§3.1 の [決定] と食い違って見えるため)。同行の**値** (`LINE Seed JP 700` / `h1` 2.5rem〜`h6` 1rem / lh 1.3)・状態 (`bound (§3.1)`) は不変。**指摘 3 に対応**し、§3 の表の Display 行の用途欄に欠落していた「**特集**」を補い 4 項目 (Display・Hero・特集・詳細施設名) へ揃えた (§3.1 の 3 行・`components.md` の [事実]・`semantic.travel.json` の `font.heading.family` の `$description` はいずれも 4 項目で、§3 の表の 1 行のみ 3 項目だった。本 Task で同行を書体選択の規則の参照先としたため、§3.1 の [決定] が挙げる許可の条件が参照先の §3 から読み取れない状態になっていた)。同行の**値** (`Noto Serif JP (明朝) 500–600`)・状態 (`bound (Q8)`) は不変。**指摘 4 に対応**し、Task 009-59 で追加した「サイズ・太さ・行間が既定から外れる場合は当該**画面仕様**または当該 Component 仕様に明示的に記載する」の 1 行を **2 行に分けて書き換えた** — ①既定から外れることは**原則として認めない**とし、例外を (a) §8.3 の**適用外 (クリエイティブ)**、(b) LP 等**利用目的・必然性が明確に示され承認を得たもの** の 2 つに限定し、いずれも §8.3 の扱い (必然性・制約・目的の提示／カラー厳守／品質下限／事前の宣言) に従うこと、承認の主体・手続は §8.3 の宣言運用によるものとし本節では定めないこと。②§8.3 の**適用**にあたる場合で Component が既定から外れる値を必要とするときは当該 **Component 仕様**に明示記載すること (Component について本節が既に定める扱いと**同じ原則**を及ぼしたものである旨へ表現を是正。当初の「同一の規則」は記載先に画面仕様を加えている点で不正確だった)。背景は、記載先とした「画面仕様」が [../screen-requirements/README.md](../screen-requirements/README.md) §15 のとおり多くが未定義で記載主体も未定であり、[README.md](README.md) §4 (上流にない画面仕様・UI 要件を Design System から逆算しない) とも整合しなかったこと。あわせて**未確定事項の一覧の当該行**を、実査の対象が「明示記載の有無」から「既定から外れている箇所の例外要件 (§8.3 の適用外に該当するか／利用目的の提示と承認を得ているか) の充足状況」となるよう改めた。**§8.3 の本文は不変** (適用/適用外の 2 段・列挙・カラー厳守・品質下限・宣言運用・グラデーション審査の各 [決定] を一切変更していない。LP を §8.3 の適用外の列挙へ追加してもいない)。本判断の正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §26-3 (Web部責任者判断 2026-09-05)。影響度は Task 009-59 と同じく**低** (判定者 = Web部責任者・判定日 2026-09-04)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
| 2026-09-05 | Task 009-59R2: Task 009-59R の記述是正 (PR [#164](https://github.com/tocoo/coocom-design-system/pull/164) の 2 回目のコードレビュー指摘 6 件に対応)。**§3.1 の [決定] を 4 行へ整理** — ①**例外の書き方を「(a)(b) の 2 つに限る」という閉じた列挙から、単一の基準＋例示へ改めた**。既定 (サイズ・太さ・行間) から外れることは原則として認めず、認めるのは**利用目的とデザイン上の必然性が明確に示され、承認を得た場合**に限る (**承認主体 = Web部責任者**)。§8.3 の適用外にあたるもの (PR 帯・特集帯・企画のキービジュアル・支給バナー・テーマタイル) や LP は**その例**であり、許容される場面をこの 2 つに限定しない。§8.3 の適用外にあたる場合は同節の扱い (必然性・制約・目的の提示／カラー厳守／品質下限／事前の宣言) にも従う。是正の理由は 2 点で、(i) 従前は「承認の主体・手続は §8.3 の宣言運用による」としていたが、§8.3 の運用は「どちらの段かを先に宣言する／宣言がなければ適用とみなす」という**自己宣言**であって承認行為ではなく、審査主体の設置は同節の [事実] のとおり未了であるため、承認を求める相手が本書内に存在しなかった。(ii) §8.3 の適用外は閉じた列挙で **LP を含まない**ため、LP を含む例外について「いずれの場合も §8.3 の扱いに従う」と書くと 1 つの [決定] の中で矛盾していた。②**Task 009-59R で追加した「§8.3 の適用にあたる Component は当該 Component 仕様へ明示記載する」の 1 行を削除**し、本節末尾の既存 [決定] (Component 側で文書レベルの既定から外れる値が必要な場合は当該 Component 仕様へ明示記載する) に**一本化**した。同行は直前の [決定] が「例外は 2 つに限る」としていたことと食い違い、かつ既存 [決定] と重複していた。Component の逸脱に別途の承認は要求しない (Component 仕様は Design System の成果物であり、記載の時点で影響度判定・内容レビューを通るため。Web部責任者判断 2026-09-05)。③「サイズ・太さ・行間の既定は書体の選択と独立」の [決定] の末尾参照を、削除した行を指さない形へ是正した。**§3 の表**の Display 行の用途欄に、**太さの優先関係**を注記した (「500–600」は書体の用途に関する記述であり、文書レベルの見出し `h1`〜`h6` の太さの既定は §3.1 の 700)。同表だけを読むと明朝を選択した `h1` に 500–600 を規範値として当てられる状態で、h1〜h6 行にのみ「既定。」の注記を入れた片側対応になっていた。同行の**値** (`Noto Serif JP (明朝) 500–600`)・**状態** (`bound (Q8)`) は不変。**未確定事項の一覧の当該行**の例外要件を新しい基準 (利用目的とデザイン上の必然性が示され、承認 = Web部責任者 を得ているか) へ揃えた。**2026-09-05 Task 009-59R の変更履歴行を是正** — 同行が指摘 1 対応の内容として記していた「既定から外す場合は既存規則どおり当該画面仕様または当該 Component 仕様へ明示記載」は、同じ行の指摘 4 対応部分が撤回したと述べる文言であり 1 行の中で自己矛盾していたため、§3.1 の他の [決定] による旨と当初版からの経緯へ書き換えた。あわせて同行が「Display・Hero・詳細施設名」と 3 項目で書いていた箇所を、実際の §3.1 の記述にあわせ **4 項目 (Display・Hero・特集・詳細施設名)** へ是正した。**`.vscode/settings.json` を本 PR から削除** (コミット `f2651f2` に Peacock 拡張が生成した個人のウィンドウ配色 29 行が誤って同梱されていた。本 Task の内容と無関係であり、コミットメッセージにも変更履歴にも記載がなく、`.gitignore` が本 Repository に存在しないため各人の設定で衝突する。作業者のローカルファイルは残し Git の追跡からのみ外した)。**不変**: §3.1 の既定値そのもの (`h1` 2.5rem 〜 `h6` 1rem・太さ 700・行間 1.3・既定書体 = ゴシック)、Task 009-59 で追加した [決定] (要素と書体の対応を固定しない) と [事実] (該当判断は本書で行わない)、Task 009-59R で追加した [決定] (既定は書体の選択と独立) の本文、§3 の表の他行と Display 行の値・状態、§3.1 末尾の既存 [決定] (`font.heading.*` の共用と Component の明示記載)、**§8.3 の本文** (適用/適用外の 2 段・列挙・カラー厳守・品質下限・宣言運用・グラデーション審査の各 [決定] を一切変更していない。LP を §8.3 の適用外の列挙へ追加してもいない)、`font.heading.*` / `font.display.*` の `$value`・参照先・`$status`、`semantic.travel.json`・`primitive.travel.json`・`components.md`・`preview.travel.html`、§1〜§2.3・§4〜§9 の他節、version 表記、rental-car / inbound の成果物。**作成・追加していないもの**: 新規 primitive・新規 token、承認手続 (申請様式・記録先・期限) の定義、§8.3 の審査基準・審査主体の設置 (Task 009-55 の範囲)、画面別の見出し階層・semantic role の割当、実装側の適用作業、`.gitignore`。本判断の正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §26-1 「是正後の承認対象 (2026-09-05)」・§26-3 (Web部責任者判断 2026-09-05)。影響度は Task 009-59 と同じく**低** (判定者 = Web部責任者・判定日 2026-09-04)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
| 2026-09-05 | Task 009-60: **C カテゴリ・特集ラベルの用途色を確定**した ([Issue #162](https://github.com/tocoo/coocom-design-system/issues/162)・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27・Web部責任者判断 2026-09-05)。§24 が「accent の帰属 (B 有料会員 / C 特集) が未定」を理由に据え置いていた C の用途色について、**accent を面として持つのは B 会員種別のみ・C は accent をアイコン (点) として持つ**という役割分離で帰属を確定し、C を白面 + 濃色文字 + accent アイコンとした。**§2 の用途 (Semantic) 要約表に 1 行追加** — `color.label.category.surface` / `text` / `icon` (surface.default / text.strong / accent.campaign = `#FFFFFF` / `#212121` / `#E4572E`)。**§2.1 の検証表に 1 行追加** — `color.surface.default` `#FFFFFF` × `color.text.strong` `#212121` = **16.10:1** (本 Repository で WCAG 2.x 標準式により実測。通常テキスト 4.5:1・大きなテキスト 3:1 とも達成)。この行により C は 12px でもサイズ・ウェイトの条件なく成立し、(i) (ii) の代替 2 択 (① neutral dark 面 / ② accent 淡色面) に依存しない。**§2.1「非操作の点的ラベルに限る白文字の例外」(Task 009-60R で「A 割引率ラベルに限る白文字の例外」へ改称) の適用範囲を縮小** — 用途を **A 割引率ラベル (`color.label.discount`) に限り**、面を **scheme 逆色 (`color.scheme.main.inverse` / `sub.inverse`) に限る**形へ改め、当初版が含めていた用途例「カテゴリラベル等」と面 `color.accent.campaign` を外した。あわせて縮小の経緯を [事実] 1 行として明記した。縮小前は、同条項が「用途にカテゴリラベルを含む・面に campaign accent を含む・サイズ/ウェイト不問」としていた一方で `components.md` `Card.slot.badge` の (i) (ii) 代替は「campaign accent 面を使用しない・代替は 2 択」と断定しており、**実装 (`tocoo/tocoo_travel` のサイト共通 `.gl-label--campaign` = accent 濃色面 + 白文字 12px) が規則違反か否かで二通りの読みが成立する状態**だった。本縮小により campaign accent 面の (i) (ii) 白文字禁止は例外なく維持される。**未確定事項の一覧**を更新 — 「C カテゴリ・特集ラベルの用途色 (`color.label.category`)」を 🚧 据え置き → ✅ **確定**へ、「バッジ/ラベルの白文字非準拠運用」の行に 2026-09-05 の適用範囲縮小を追記。**1 行追加** — 白面の C ラベルを白系の面 (`surface.default` / `subtle`) の上に置く場合の分離手段 (境界線の要否・値) が ❓ 未定義であること (本 Task で C の面を白と確定したことから派生。決定主体 = Web部責任者・チーフデザイナー。写真上の scrim = 依頼 E-1 とは別論点)。**不変**: §2.1 の (a) (b) 分岐の境界・用語の定義・代替規則 1〜3 の本文・campaign accent 面上の文字の各 [決定]・scheme inverse 面の各 [決定]・評価色と販促面色の用途境界・検証表の既存 13 行の値、§1・§2.2・§2.3・§3〜§9、`color.membership.paid` / `color.accent.campaignTint` / `campaignInk` の値・参照先・`$status` (placeholder)・accent 濃色段と coral 2 段の 🚧 仮色、primitive の色値、version 表記、rental-car / inbound の成果物。**作成・追加していないもの**: 新規 primitive・新しい色値 (`color.label.category` の 3 件はいずれも既存 semantic への参照)、副色 (sub) 文脈での C の用途色の個別定義、白面 C ラベルの境界線の規定、b2 の 4.5:1 成立検証・placeholder の bound 昇格、実装側の適用作業。改訂着手の設計承認は [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27-1 に記録 (承認主体 = Web部責任者・承認日 2026-09-05)。影響度は**高** (判定者 = Web部責任者・判定日 2026-09-05・本件について明示取得。必要レビュー主体 = Web部責任者およびチーフデザイナー = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
| 2026-09-05 | Task 009-60R: Task 009-60 の記述是正と、白系の面の上での分離手段の確定 (PR [#165](https://github.com/tocoo/coocom-design-system/pull/165) のコードレビュー指摘 4 件に対応)。**①§2.1 の白文字例外条項を改称** — 「非操作の点的ラベルに限る白文字の例外」→「**A 割引率ラベルに限る白文字の例外**」。Task 009-60 で適用範囲を A 割引率ラベル + scheme 逆色面へ縮小した結果、旧見出しは本文の [決定]「非操作の点的ラベルであっても A 割引率ラベル以外 (B / C / D / E) は本例外の対象外」と逆の適用範囲を示していた。本見出し名は §2 の表 (`color.label.discount` 行)・未確定事項の一覧・`components.md` `Card.slot.badge`・`semantic.travel.json` の `color.label.discount` / `color.label.category` の `$note`・[preview.travel.html](preview.travel.html) から**引用名**として参照されており、引用元をたどると規則の逆を読む経路が残っていたため。本書内 3 箇所と他ファイル 4 箇所の引用を新見出し名へ更新し、条項の [事実] に改称の経緯を追記した (2026-09-05 以前の変更履歴行の旧名は歴史記録として残す)。**②Task 009-60 の変更履歴行の誤記を是正** — 不変句「検証表の既存 **14 行**の値」は本 Task で追加した 1 行を「既存」に数えていた。改訂前のデータ行数は **13 行**・改訂後が 14 行 (`git show origin/main:` の同ファイルで実測) であるため「既存 **13 行**」へ是正し、同行に見出しの改称も追記した。**③白系の面の上に置く C ラベルの分離手段を確定** (Web部責任者判断 2026-09-05・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27-3 ⓕ) — **白系の面 (`color.surface.default` `#FFFFFF` / `color.surface.subtle` `#F9F9F9`) の上に置く場合にのみ、`color.label.category.border` (= `color.border.default` `#CCCCCC`) の枠線を `border.width.thin` (1px) で引く**。写真の上・有色面の上では枠線を引かず面のみで分離する。§2 の用途要約表の C 行に `border` を加え、§2.1 の検証表の直後に [事実] 1 行を追加して**実測値**を記録した — 枠線は白面上 **1.61:1**・`surface.subtle` 上 **1.53:1** で、いずれも**非テキスト UI 要素の 3:1 に達しない** (既存 border 3 段はいずれも未達 = `subtle` 1.32:1 / `default` 1.61:1 / `strong` 1.90:1)。枠線は面のみでは境界が生じないこと (白 × 白 1.00:1・白 × `surface.subtle` 1.05:1) に対する分離の補助であり、識別は文字 (≈16.10:1) が担う。**適合宣言は行わない**。上表は文字と面の組み合わせを対象とするため、非テキストの境界である本値は表へ行として加えず [事実] として記録した (「表に無い組み合わせを検証済みとして扱わない」原則は維持)。未確定事項の一覧の当該行を ❓ 未定義 → ✅ **確定**へ更新した。**④未確定事項の一覧に 1 行追加** — C カテゴリ・特集ラベルの**アイコンのサイズ**が未規定であること。アイコン体系のサイズスケール `iconSize.sm〜xl` の最小段は **16px** で、器 sm (高さ `label.height.sm` 20px・文字 `label.fontSize.sm` 12px) に当てる値が本書・`components.md` のいずれにも無い ([preview.travel.html](preview.travel.html) の見本は継承の 12px で描いておりスケール外の例示。rental-car の C1 / C2 も同じく未規定)。**不変**: §2.1 の (a) (b) 分岐の境界・用語の定義・代替規則 1〜3 の本文・campaign accent 面上の文字の各 [決定]・scheme inverse 面の各 [決定]・評価色と販促面色の用途境界・**白文字例外の適用範囲そのもの** (A 割引率ラベル + scheme 逆色面)・検証表の全 14 行の値、`color.label.category` の `surface` / `text` / `icon` の値と参照先、§1・§2.2・§2.3・§3〜§9、`color.membership.paid` / `campaignTint` / `campaignInk` の `$status` (placeholder)、primitive の色値、version 表記、rental-car / inbound の成果物。**作成・追加していないもの**: 新規 primitive・新しい色値 (`color.label.category.border` は既存 `color.border.default` への参照)、アイコンサイズの値の決定、375px の横はみ出しの是正。影響度は Task 009-60 と同じく**高** (判定者 = Web部責任者・判定日 2026-09-05。必要レビュー主体 = Web部責任者およびチーフデザイナー = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
| 2026-09-05 | Task 009-61: **C カテゴリ・特集ラベルのアイコンの未確定事項に「字形」を加える是正** ([Issue #166](https://github.com/tocoo/coocom-design-system/issues/166))。Task 009-60R が起票した未確定行は対象を**サイズのみ**としていたが、C の意匠に含めたアイコンは**字形 (どのアイコンを用いるか) も本書・`components.md` のいずれにも定めがない** (§6 はアイコン体系を Font Awesome 6 と定め、サイズスケール `iconSize.sm〜xl` を置くのみで、個々のラベルへの字形の割当規則を持たない)。字形が未定義である事実の起票が**非正本の [preview.travel.html](preview.travel.html) 側にしか存在しない**状態だったため、未確定事項の一覧の当該行の対象を「アイコンの**字形とサイズ**」へ広げ、字形について未定義である根拠 (§6 が割当規則を持たないこと・見本の `fa-bolt` は例示であること) を明記した。**不変**: 同行のサイズに関する記述 (`iconSize` 最小段 16px・器 sm への当て値が無いこと・見本の 12px がスケール外の例示であること)・決定主体 (Web部責任者・チーフデザイナー)・rental-car C1 / C2 も未規定である事実、§6 の本文、`color.label.category.*` の `$value`・参照先・`$status`、`semantic.travel.json`・`primitive.travel.json`、§1〜§9 の他節、version 表記。**作成・追加していないもの**: 字形の割当規則そのもの (どのアイコンを用いるか)・アイコンサイズの新規トークン・新規 primitive。影響度は**低** (判定者 = Web部責任者・判定日 2026-09-05・Task 009-60 と同じ。必要レビュー主体 = Web部レビュー担当者 = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8・§10。トークンの値・`$status` に及ばず、既に未定義である状態を一覧の記述へ反映する是正であるため)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
