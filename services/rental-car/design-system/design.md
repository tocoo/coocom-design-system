...
| Modal の form の a11y・実装 API 名 | ❓ 未定義 (§7.1)。a11y は DS 層で決定しない。最小タップ領域 44px の spacing トークンは追加しない。**`sheet` の最大高 (`90vh`) と `popover` の幅 (`240〜320px`) は ✅ 確定した** (2026-09-07・Web部責任者判断・判断 F-6・正本 = §7.1) | 実装実査 / 実装側タスク |
