...
| Design System | 視覚・トークン・コンポーネント定義 | [design-system/](design-system/README.md) | 既存資産 (Draft)・上流との Alignment は Work Order 1〜6 を Draft で実施済み・Work Order 6 は Task 009-6 で再評価済み (全候補「現時点では開始できない」を維持・Alignment 未完了・実際の改定は未着手) |
...
Design System の既存資産 (`design.md`・JSON・`components.md`) は本 bootstrap 以前から存在し、内容は変更していない (移行に伴う参照パス修正を除く)。入口 README (`Draft`) は既存資産と上流成果物の関係を管理する基盤であり、既存資産の内容・version・Status を変更しない。上流との Alignment 評価は Work Order 1〜6 (Baseline 確認・Service Design 横断整合性評価・Screen Requirements 個別整合性評価・Observation 横断集約・改訂候補と未解決事項の分離・改訂着手可否の判断) を Draft で実施済みであり、Work Order 6 は Task 009-6 で再評価済みである。Work Order 6 とその再評価は全候補側面を「現時点では開始できない」と記録し、実際の Design System 改訂タスクは現時点では開始できないと総合判断した (再評価では旧共通阻害 Fact が Review / Approval Rules で定義された範囲において解消したが、候補固有の阻害 Fact が残る。§19 Alignment 完了条件は未充足で Alignment は未完了)。着手可否評価・その再評価の記録は Alignment 承認・完了や改定完了を意味せず、実際の Design System の改定・候補採否・改訂着手の設計承認は本 PR のマージと必要なレビュー・Web部責任者の明示的判断の完了を経た別タスクでのみ扱う (改定を既定路線としない)。
