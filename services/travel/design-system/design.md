...
| — | 割引率等のコンテンツ表記規則の管理正本 (独立文書を新設する方向を確定) | ✅ 方向確定 (依頼 E-4・Q11・Web部責任者判断 2026-08-03、正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §16)。独立文書 (`brand-content.md` 等) を新設する方向。実際の作成・中身・管理責務は別 Task (参照切れの `brand-content.md` は推測で新規作成しない = README.md §16) |
...
| — | 会員限定 (有料会員限定) ラベルをカテゴリラベルと区別するか (`Card.slot.badge`) | 🚧 方向確定 (依頼元 2026-08-04・依頼A・Web部責任者判断 2026-08-04、正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §19)。**区別する**・手段 = 面色の使い分け (基本 neutral 面 / 特集紐づき accent 面 / 例外 inverse 面)・**既存トークンの範囲で成立する組み合わせに限定**。面色・文字色の成立条件は §2.1 の検証表が正で Task 009-44 と重なる。写真上への配置は Task 009-46 の scrim (E-1) と関連。`会員限定` の**マスク表現** (E-2・上記行) とは別論点。用途トークン・区別規則の実定義は改訂着手の設計承認 (§9・§20) を経た別 Task |
| — | 会員種別語 (無料 / 有料会員) の表記管轄 | ❓ コンテンツ表記規則の管轄 (DS の token 管轄外・依頼元 2026-08-04・依頼A)。管理正本の所在は §16 の Open Issue に属する (現状 = 独立文書 `brand-content.md` 新設方向・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §16。Task 009-45 で design.md §8 系集約へ撤回する方向を別途判断)。本書では会員種別語の具体表記を確定しない (§19) |
...
| 2026-08-04 | Task 009-43: 依頼元 (2026-08-04・依頼A + 判断点7) に対応 (受理と分類)。**未確定事項の一覧に 2 件追加**した — 会員限定 (有料会員限定) ラベルをカテゴリラベルと区別するか (🚧 方向確定 = 区別する・手段 = 面色使い分け 基本 neutral / 特集 accent / 例外 inverse・既存トークン範囲・実定義は別 Task・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §19)・会員種別語 (無料 / 有料会員) の表記管轄 (❓ コンテンツ表記規則の管轄・管理正本の所在は §16 論点 = `brand-content.md` 新設方向で Task 009-45 が撤回方向を別途判断)。**不変**: §1〜§9 の本文 (§2.1 面色規則・§8 系を含む)、既存 token の値・参照先・`$status`・`$meta.version`・version、`color.text.onAccent` (既 `bound`)、primitive の色値、Component の実装要件、rental-car / inbound の成果物。**行っていないもの**: 会員限定ラベルの用途トークン・区別規則の実定義・`Card.slot.badge` 規範規則の変更・新色値/新面色用途の追加・`brand-content.md` の新規作成・依頼元実装の追認。影響度 = **低** (判定者 = Web部責任者、判定日 = 2026-08-04、本件について明示取得。必要レビュー主体 = Web部レビュー担当者)。改訂着手の設計承認は取得していない (§9・§20)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
