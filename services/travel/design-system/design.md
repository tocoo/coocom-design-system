...
| `color.label.discount.surface` / `text` (A 割引率) | scheme.main.inverse / text.inverse | `#C8912C` / `#FFFFFF` | bound (2.78:1・§2.1「非操作の点的ラベルに限る白文字の例外」・AA 未達明示) |
| `color.label.category.surface` / `text` / `icon` (C カテゴリ・特集) | surface.default / text.strong / accent.campaign | `#FFFFFF` / `#212121` / `#E4572E` | bound (白面 × 濃色文字 ≈16.10:1・accent は**アイコン (非テキスト 3:1)** に限る ≈3.68:1・§2.1) |
...
| — | バッジ/ラベルの白文字非準拠運用 (割引率 `#C8912C` 面/白 14px・小サイズ campaign accent 面/白 12px) の扱い | ✅ **実規則化済み** (Task 009-48・ラベル・タグ定義シート A・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。§2.1「非操作の点的ラベルに限る白文字の例外」条項・`components.md` `Card.slot.badge` の例外条項として追加。**非操作の点的ラベル + campaign accent/scheme 逆色面 に限り白文字を例外許容** (サイズ/ウェイト不問・AA 未達明示・`#C8912C`×白 2.78:1 等)。白文字が成立する面は `color.surface.inverse` (16.10:1) の 1 面のみ。原則 (白文字禁止) は維持し例外を指定面に限る。b2 (accent 淡色面) は別経路として併存・実色値待ち。**2026-09-05 に適用範囲を縮小** — Task 009-60 (Web部責任者判断・[../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §27) で C カテゴリ・特集ラベルを白面へ確定したことに伴い、例外の用途を **A 割引率ラベルに限り**、面を **scheme 逆色に限る** (campaign accent 面 + 白文字は例外から外し、(i) (ii) の白文字禁止を例外なく維持) |
