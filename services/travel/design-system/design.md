...
上記の「campaign accent 面上の文字」「scheme inverse 色を面として使用する場合」は、いずれも白文字を通常利用として許可しない (campaign accent 3.68:1 は (a) (b) のみ・scheme 逆色 2.78:1 / 2.15:1 は禁止)。これに対し、次の限定条件を**すべて**満たす場合に限り、白文字を**例外**として許容する (AA 未達を明示する)。

- [決定] **用途 = 非操作の点的ラベル** (バッジ・割引率ラベル・カテゴリラベル等の非操作ラベル) に限る。操作可能なバッジは badge ではなく action 系 Component として扱い、本例外の対象外とする
- [決定] **面 = campaign accent (`color.accent.campaign` `#E4572E`) または scheme 逆色 (`color.scheme.main.inverse` `#C8912C` / `color.scheme.sub.inverse` `#C8B12C`)** に限る。`color.surface.inverse` (`#212121`・16.10:1) 以外へ拡大しない (白文字が通常テキスト 4.5:1 を満たす面は `surface.inverse` の 1 面のみ)
- [決定] サイズ・ウェイトは**不問** (小サイズ・通常ウェイトを含む)。ただし**コントラストが AA (通常テキスト 4.5:1) に未達であることを明示する** (campaign accent × 白 3.68:1・main 逆色 × 白 2.78:1・sub 逆色 × 白 2.15:1)。適合宣言は行わない
- [決定] 本例外は現行の「`color.scheme.*.inverse` を面として使用する場合の文字色は `color.text.strong` に固定・白文字を使用しない」原則および campaign accent 面の (i) (ii) 白文字禁止に対する**限定的な例外**であり、原則そのものは維持する (例外を非操作の点的ラベル + 指定面を超えて拡大しない)
...
| — | バッジ/ラベルの白文字非準拠運用 (割引率 `#C8912C` 面/白 14px・小サイズ campaign accent 面/白 12px) の扱い | ✅ **実規則化済み** (Task 009-48・ラベル・タグ定義シート A・正本 = [../../../governance/owner-decisions.md](../../../governance/owner-decisions.md) §24)。§2.1「非操作の点的ラベルに限る白文字の例外」条項・`components.md` `Card.slot.badge` の例外条項として追加。**非操作の点的ラベル + campaign accent/scheme 逆色面 に限り白文字を例外許容** (サイズ/ウェイト不問・AA 未達明示・`#C8912C`×白 2.78:1 等)。白文字が成立する面は `color.surface.inverse` (16.10:1) の 1 面のみ。原則 (白文字禁止) は維持し例外を指定面に限る。b2 (accent 淡色面) は別経路として併存・実色値待ち |
...
| — | C カテゴリ・特集ラベルの用途色 (`color.label.category`) | 🚧 仮決定・据え置き (Task・依頼 §10)。accent の帰属 (B 有料会員 / C 特集) が確定するまで暫定。仮意匠 = 白面 + 濃文字 + accent アイコン。DS への用途色追加は accent 帰属確定後 |
