...
- `Card.slot.badge` (割引率ラベル・状態バッジ・カテゴリラベル等の非操作ラベル) の規則:
  ...
  - ラベル種別と用途色 (面色の成立条件は上記分岐・`design.md` §2.1 / §8.2 / §8.3 が正):
    - **A 割引率** = `{color.label.discount}` (逆色面 #C8912C + 白文字・非操作の点的ラベルの例外)
    ...
    - **C カテゴリ・特集** は仮決定 (accent の帰属未定) のため用途色は据え置き (`governance/owner-decisions.md` §24)
  ...
  - 割引ラベルの面色 (c1・Web部責任者判断 2026-08-03。`design.md` §2.1 が正):
    ...
    - **[例外] 非操作の点的ラベルに限る白文字** (Task 009-48・A 割引率ラベル = `{color.label.discount}`): 逆色面 (`{color.scheme.*.inverse}`) または campaign accent 面 + 白文字 (`{color.text.inverse}`) を、**非操作の点的ラベルに限り例外として許容する** (逆色面 2.78:1 / 2.15:1・campaign accent 3.68:1 = **AA 未達を明示**・サイズ/ウェイト不問)。白文字が通常テキスト 4.5:1 を満たす面は `{color.surface.inverse}` の 1 面のみ。操作可能なバッジは対象外 (action 系)。原則 (白文字禁止) は維持し例外を指定面に限る (`design.md` §2.1「非操作の点的ラベルに限る白文字の例外」が正)
