...
- [決定] 上記の明示的な選択の結果、サイズ・太さ・行間が本節の既定から外れる場合は、当該**画面仕様または当該 Component 仕様に明示的に記載する** (暗黙の上書きを認めない)。これは本節が Component の見出しについて定める扱い (`font.heading.*` の共用と逸脱の明示記載) と同一の規則であり、文書レベルの見出しにも及ぶ
- [決定] **サイズ・太さ・行間の既定は書体の選択と独立に定める。** 書体として明朝 (`font.display.family`) を明示的に選択しても、本節の既定 (`h1` 2.5rem 〜 `h6` 1rem・太さ 700・行間 1.3) はそのまま適用される。§3 の表が Display・Hero・詳細施設名に記す「500–600」は書体の用途に関する記述であり、文書レベルの見出しの太さの既定を置き換えない。既定から外す必要がある場合は、上記のとおり当該画面仕様または当該 Component 仕様に明示的に記載する
...
| — | 文書レベルの見出しに明朝 (`font.display.family`) を明示的に選択している箇所の棚卸しと、既定から外れる値 (サイズ・太さ・行間) の明示記載の有無 | 🚧 実査待ち ([Issue #163](https://github.com/tocoo/coocom-design-system/issues/163))。本書は書体の選択規則のみを定め、個々の画面での該当判断・記載の有無は確定しない。実装 Repository (`tocoo/tocoo_travel`) 側の適用作業は本書の範囲外 |
