...
- [事実] リンクは色 `{color.text.link}` (= 主色・TVL-0011) に加え、文中リンクの下線と状態 (hover = `{color.text.linkHover}` / active = `{color.text.linkActive}` / visited = 専用色を設けず `{color.text.link}` を維持 / focus = `{color.focus.ring}` の `outline`) を使用する。**hover で下線を外さない**。**リンクの状態表現に `opacity` を用いない** — 上記のボタン hover の暫定「opacity 変化 (≈0.85)」(follow-up #4) はボタン状態の暫定値であり、リンクの規則として読み替えない。正は `design.md` §2.2
...
- 状態:
  - hover: 🚧 暫定 opacity 変化 (navy 暗色未抽出 = follow-up #4)。`Button.text` は文字色に `{color.text.link}` を用いるが、**その hover を `{color.text.linkHover}` と同一視しない** (ボタン状態一式は follow-up #4 で未取得。`design.md` §2.2 はリンク要素の規則)
...
- Do / Don't:
  ...
  - Do: プレースホルダは入力の補助にとどめ、ラベル・必須表現・入力形式の説明の代替にしない
...
- Do: 現在地はリンク化しない
- 未確定事項: パンくずのリンクへ下線の既定を及ぼすかは未判定 (`design.md` §2.2 が定めたのは文中リンクの既定。パンくずは領域とレイアウトでリンクが成立する standalone 側にあたる)。hover / active の文字色は `{color.text.linkHover}` / `{color.text.linkActive}` に従う
...
| 2026-07-29 | Task 009-35: **共通事項に 3 行追加**した。①見出しは文書レベル (h1〜h6) と Component で同一トークン群 `{font.heading.*}` を共用し、割当 (h1 2.5rem / h2 2rem / h3 1.5rem / h4 1.25rem / h5 1.125rem / h6 1rem・ウェイト 700・行間 1.3) は `design.md` §3.1 が正であること、Component が既定から外れる値を必要とする場合は当該 Component の項へ明示記載すること (暗黙の上書きを認めない)、明朝は明示的に選択する書体であり見出しの既定ではないこと。②リンクは色に加え文中リンクの下線と状態 (hover = `{color.text.linkHover}` / active = `{color.text.linkActive}` / visited = 専用色なしで `{color.text.link}` 維持 / focus = `{color.focus.ring}` outline) を使用し、hover で下線を外さないこと、リンクの状態表現に `opacity` を用いず**ボタン hover の暫定 opacity 変化 (≈0.85・follow-up #4) をリンクの規則として読み替えない**こと。③プレースホルダ文字色は `{color.text.placeholder}` (#616161・≈6.2:1) を使用し `{color.text.muted}` (#9e9e9e・≈2.7:1) を流用しないこと、必須表現・ラベル・エラーメッセージ・入力形式説明の代替に用いないこと。**Button** の hover 行に、`Button.text` は文字色に `{color.text.link}` を用いるがその hover を `{color.text.linkHover}` と同一視しない旨 (ボタン状態一式は follow-up #4 で未取得) を追記した。**Input** に構成行 (入力値 `{color.text.body}` / プレースホルダ `{color.text.placeholder}`) を追加し、Do 1 行・Don't 1 行を追加、関連トークン行を `color.text.*` から `color.text.body` / `color.text.placeholder` へ具体化した。**Breadcrumb** に未確定事項 1 行を追加した (パンくずのリンクへ下線の既定を及ぼすかは未判定 = standalone 側。hover / active の文字色は `{color.text.linkHover}` / `{color.text.linkActive}` に従う)。**不変**: 共通事項の既存行、SearchForm / Card / PriceTag / ReviewStars / Header / Footer / Modal の仕様、Button のバリアント・Do / Don't・関連トークン・未確定事項、Input のステータス・用途・バリアント・状態 (error / focus / disabled / success の 🚧)・未確定事項 (follow-up #2)、Breadcrumb の構成・Do、`{color.text.link}` / `{color.text.muted}` / `{font.heading.family}` / `{font.heading.h2Size}` の値・参照先・`$status`、primitive の値、version 表記、UI 構造・Component API・DOM 構造、rental-car / inbound の成果物。**行っていないもの**: 状態固定リスト (命名規則§2) への追加、variant 語彙 (GOV-0002) への追加、新規 primitive の追加、reset / base 層の CSS 実装方針の策定、実装ファイルの変更。影響度は**未取得** (判定主体 = Web部責任者の都度判断 = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8)。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
