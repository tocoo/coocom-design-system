...
| 既定から外れる値の可否 (2026-09-05) | **原則として認めない**。サイズ・太さ・行間は §3.1 の既定に従う。例外として許容するのは、(a) `design.md` §8.3 の**適用外 (クリエイティブ)** に該当するもの、および (b) LP 等、**利用目的・必然性が明確に示され承認を得たもの**に限る。「当該画面仕様に明示的に記載する」という当初文言は、例外の基準を示さないまま記載先を上流成果物に求めるものであったため、本判断により §8.3 の適用/適用外の 2 段と承認の有無を基準とする形へ改めた。承認の主体・手続は §8.3 の宣言運用によるものとし §3.1 では定めない。取得の契機 = PR #164 のコードレビュー指摘 (記載先の「画面仕様」が [../services/travel/screen-requirements/README.md](../services/travel/screen-requirements/README.md) §15 のとおり多くが未定義で、記載主体も未定) |
