...
| Display・Hero・特集・詳細施設名 (書体選択の規則 = §3.1) | Noto Serif JP (明朝) 500–600 | **bound (Q8)** |
...
- [決定] 上記の明示的な選択の結果、サイズ・太さ・行間が本節の既定から外れることは**原則として認めない**。例外として許容するのは、(a) §8.3 が定める**適用外 (クリエイティブ)** に該当するもの (PR 帯・特集帯・企画のキービジュアル・支給バナー・テーマタイル)、および (b) LP 等、**利用目的・必然性が明確に示され承認を得たもの**に限る。いずれの場合も §8.3 の扱い (必然性・デザイン上の制約・目的の提示／**カラーは厳守**／品質の下限／どちらの段かの事前の宣言・宣言がなければ適用とみなす) に従う。承認の主体・手続は §8.3 の宣言運用によるものとし、本節では定めない (審査・承認主体の設置は同 §8.3 の [事実] のとおり別論点)
- [決定] §8.3 の**適用** (UI 部品・カード・ラベル全般) にあたる場合で、Component が文書レベルの既定から外れる値を必要とするときは、当該 **Component 仕様に明示的に記載する** (暗黙の上書きを認めない)。これは Component の見出しについて本節が既に定める扱い (`font.heading.*` の共用と逸脱の明示記載) と同じ原則を、文書レベルの見出しへ及ぼしたものである
- [決定] **サイズ・太さ・行間の既定は書体の選択と独立に定める。** 書体として明朝 (`font.display.family`) を明示的に選択しても、本節の既定 (`h1` 2.5rem 〜 `h6` 1rem・太さ 700・行間 1.3) はそのまま適用される。§3 の表が Display・Hero・特集・詳細施設名に記す「500–600」は書体の用途に関する記述であり、文書レベルの見出しの太さの既定を置き換えない。既定から外れる場合の扱いは上記 2 行 (原則として認めない・例外は §8.3 の適用外または承認を得たものに限る・§8.3 の適用にあたる Component は当該仕様へ明示記載) による
...
| — | 文書レベルの見出しに明朝 (`font.display.family`) を明示的に選択している箇所の棚卸しと、既定 (サイズ・太さ・行間) から外れている箇所の例外要件 (§8.3 の適用外に該当するか／利用目的の提示と承認を得ているか) の充足状況 | 🚧 実査待ち ([Issue #163](https://github.com/tocoo/coocom-design-system/issues/163))。本書は書体の選択規則のみを定め、個々の画面での該当判断・記載の有無は確定しない。実装 Repository (`tocoo/tocoo_travel`) 側の適用作業は本書の範囲外 |
