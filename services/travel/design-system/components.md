...
- [事実] 面 (背景) として色を使う場合の許可される文字色・文字サイズ条件は面ごとに異なる。組み合わせ一覧・campaign accent を面として使用できる条件 (`24px` 以上の通常ウェイト、または `20px` 以上の bold)・それ以外の場合の代替規則・scheme inverse 面 (main `#C8912C` / sub `#C8B12C`) の文字色は `design.md` §2.1 が正。**`{color.text.onAccent}` の存在を理由にコントラスト確認を省略しない**
...
- `Card.slot.badge` (割引率ラベル・状態バッジ・カテゴリラベル等の非操作ラベル) の規則:
  ...
  - 面と文字色は文字サイズ・ウェイトで分岐する (境界の定義は `design.md` §2.1 が正)
    - **`24px` 以上の通常ウェイト、または `20px` 以上の bold**: 面 `{color.accent.campaign}` (#E4572E) + 文字 `{color.text.onAccent}` (≈3.68:1・大きなテキスト基準 3:1 のみ達成)
    - **上記以外** (= `20px` 未満のすべて／`20px` 以上 `24px` 未満の通常ウェイト): 面 `{color.accent.campaign}` を使用しない。既定は 面 `{color.surface.inverse}` (#212121) + 文字 `{color.text.inverse}` (≈16.10:1)。accent は境界色・アイコン・点的装飾に限定する (**accent を文字色として明色面に置く方法はこの場合の代替にならない** = 明色面上 3.38〜3.68:1 で通常テキスト 4.5:1 未達)
