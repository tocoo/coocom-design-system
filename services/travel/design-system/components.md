...
- 構成:
  ...
  - 未選択値 — `{color.text.muted}` (`#9E9E9E`・白背景 **2.68:1**・**AA 未達**。`design.md` §2.3 の例外条項が正)
...
- Do / Don't:
  ...
  - Don't: 未選択値の `{color.text.muted}` を、`input` / `textarea` の `::placeholder` へ広げない (そちらは `{color.text.placeholder}` を維持する = `design.md` §2.3)
