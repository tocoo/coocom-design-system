...
| 項目 | 内容 |
| --- | --- |
| 承認対象 | ①[../services/travel/design-system/semantic.travel.json](../services/travel/design-system/semantic.travel.json) への用途色 `color.label.category` (`surface` / `text` / `icon` の 3 件) の追加、②[../services/travel/design-system/components.md](../services/travel/design-system/components.md) `Card.slot.badge` の「ラベル種別と用途色」C 行の据え置き記述の置換と白文字例外条項の適用範囲の縮小、③[../services/travel/design-system/design.md](../services/travel/design-system/design.md) §2 用途要約表への 1 行追加・§2.1 検証表への 1 行追加・§2.1「非操作の点的ラベルに限る白文字の例外」の適用範囲の縮小・未確定事項の一覧の更新 (C 用途色を解決へ・派生する未定義事項 1 行の追加) |
| 承認種別 | **改訂着手承認** ([review-approval-rules.md](review-approval-rules.md) §9・§20)。候補採否とは別の判断 |
| 承認日 | 2026-09-05 |
| 承認主体 | Web部責任者 |
...
- **白面の C ラベルを白系の面 (`surface.default` / `subtle`) の上に置く場合の分離手段** (境界線の要否・値)。本記録は C の面を白と確定するが、白いカード面・白い背景の上での境界の与え方は定めない (`design.md` 未確定事項の一覧へ 1 行として記録する)。画像・グラデーションを面とする場合の scrim の要否 (依頼 E-1) は従前どおり別論点。
