...
本 README は Design System レイヤーの入口文書であり、個別のトークン・Component・ブランド値・Design Decision そのものは定義・変更しない。既存 Design System 資産と上流成果物の関係を管理するための責務・原則・境界・Open Issue を定義する。具体的な Alignment Assessment (上流要件と既存資産の対応評価) の実行記録は本 README ではなく後続の Work Order 成果物 (Work Order 1〜5、いずれも Draft) が担い、本 README 自身は個別の対応評価結果を定義する正本ではない。Work Order 1〜5 の実施は上流整合の承認・完了を意味せず、改訂着手可否の判断 (Work Order 6) と Design System の改定は未着手である。
...
- 既存 Design System が上流成果物と整合しているか、作成済みの個別 Screen Requirement を満たせるかは、本 README では判定しない (整合性評価の実行記録は Work Order 1〜5 の各 Draft 成果物にあり、本 README はその結果の正本ではない。承認・Alignment 完了は未決)。
...
- 既存 Design System が SD-001〜SD-007 と整合しているか、作成済みの個別 Screen Requirement を満たせるかの整合性評価は、Work Order 1〜5 の各 Draft 成果物として実施済みである (§7・§10・§15)。その承認・Alignment 完了、および改訂着手可否の判断 (Work Order 6)・Design System の改定は未決・未着手である。
- Alignment Assessment の計画文書 [alignment-plan.md](alignment-plan.md) が Draft として存在し、その計画に基づく Alignment Assessment (Work Order 1〜5) は各 Draft 成果物として実施済みである (§7・§10)。
...
- 既存 Design System 資産の内容を「正しい」「承認済み」「上流整合済み」「Complete」として扱わない。既存資産が Draft であることと、上流整合が承認・完了していないこと (整合性評価は Work Order 1〜5 の Draft 記録として存在するが、承認・Alignment 完了には至っていない) を区別する。
...
- 具体的な改訂・追加・廃止 (Design System の改定) は後続タスク (Work Order 6 以降) で行い、本 README では着手・決定しない。対応・gap の Alignment Assessment は Work Order 1〜5 の Draft 成果物として実施済みで、その結果を本 README では確定しない (§10)。
...
- 具体的な改訂・追加・廃止 (Design System の改定) は後続タスクで行う (§10。対応・gap の Alignment Assessment は Work Order 1〜5 で Draft 実施済み)。
...
| 2026-07-17 | Align 冒頭本文・§1・§3 Facts/Decisions・§4 の Alignment Assessment 状態記述を Work Order 1〜5 実施済みの現状へ整合是正 (PR #55 補足コメントへの対応。旧「後続タスク／未実施／未評価」表現を、実行記録が Work Order 1〜5 の Draft 成果物にある旨へ是正)。本 README が個別評価結果の正本でない責務境界・Design System 改定を README で行わない境界・Work Order 6 と改定が未着手である Fact・承認/Alignment 完了が未決である Fact は維持。評価結果・Decision・UX Decision の追加なし | Draft |
