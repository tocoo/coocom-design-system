...
- [事実] テキスト色は `{color.text.strong}` / `{color.text.body}` / `{color.text.mutedStrong}` / `{color.text.muted}` の4段。判読性を必要とする補助情報 (補助価格・税/人数/泊数等の条件注記・割引前価格) は `{color.text.mutedStrong}` (#616161・白背景 ≈6.2:1) を使用する。`{color.text.muted}` (#9e9e9e・白背景 ≈2.7:1) は通常テキストに求められる 4.5:1 に達しないため、判読性を要する情報には用いない (design.md §2)
...
- 構成:
  ...
  - 補助価格・条件注記 (税・人数・泊数等の条件・割引前価格): `{font.price.captionSize}` (0.75rem) / `{color.text.mutedStrong}`
- Do / Don't:
  - Do: 税・条件の補助テキストを必ず併記できる構造にする
  - Do: 補助価格・条件注記の文字色は `{color.text.mutedStrong}` (#616161・白背景 ≈6.2:1) を使用する
  - Don't: 数字と単位を1つのテキストで固定しない (スロット分離)
  - Don't: 購買判断・内容理解に必要な補助テキストへ `{color.text.muted}` (#9e9e9e・白背景 ≈2.7:1) を使用しない
