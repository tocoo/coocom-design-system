...
- **T1 完了条件 (§8A.2 T1) の評価**: 完了条件は「Global Navigation 対象領域と History and Recovery 保持方針についての明示的判断・更新が `navigation.md` および該当 Screen Requirements へ記録されること」であった。本 Task の明示的判断は **「これらは Service Design 層で固定する必要のない事項であり、上流仕様として確定しない → 上流成果物の変更は不要」** である。この判断を本トラッキング文書 (§8C) へ記録した。`navigation.md` への固定記載は、案件判断への結合と変更時の保守負担を生むため不適切と判断し、行わない。したがって T1 は「上流成果物への固定記載を要しない」という形で明示的判断が記録され完了とする (上流変更なしが結論)。
- **解決した事項**: `navigation.md` §10 の Open Issue「Global Navigation の対象領域」「履歴・文脈保持方式」は、**上流で固定する必要のない事項**と整理された (上流仕様としての確定を要しない)。ただし設計正本を触らない方針のため、`navigation.md` 本文の Open Issue 記載自体は本案件で変更しない。
- **未確認として残る事項 (推測しない)**: 現行 Global Navigation の具体的な対象領域・項目・表示順・ラベル・遷移先／History and Recovery の現行実装が実際に保持する条件・文脈・保存方式・保存期間・セッション仕様・ブラウザバック等の具体挙動。いずれも直接証拠がない。ただし「固定する必要がない」という整理のため、これらは上流確定の前提条件ではない。
...
- **T1 の完了状態と残る依存関係の評価**: T1 は「Service Design 層で固定する必要のない事項」と整理され、上流成果物の変更は不要と結論づけた。T1 固有に残る未確認事項 (現行仕様の直接証拠) はあるが、「固定不要」という整理により、それらを解消しないと T1 を先へ進められない、という依存関係は生じない (上流固定を前提としないため)。よって T1 固有の追加解消タスクを優先候補としない。
