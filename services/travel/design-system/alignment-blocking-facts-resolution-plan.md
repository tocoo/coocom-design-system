...
- **影響度**: 高 (Web部責任者判定、2026-07-22)。Web部責任者の恒常方針「ルール制定段階で軽微 (＝PR #83 の編集的訂正 carve-out) でなければすべて高」に基づき、本工程は実質的な再分類で軽微でないため高。理由 = 阻害 Fact の意味・分類・後続工程・Design System 候補側面 §6.9 との関係に影響し得る。PR #83 の carve-out は本工程に適用しない。必要レビュー主体 = Review / Approval Rules §10 既定の Web部責任者 ＋ チーフデザイナー。
...
| --- | --- | --- | --- | --- |
| Search Criteria の指定・確認・変更・文脈保持・回復・状態識別の**責務** | (1)(2)(9)(10)(12) | 既存 Draft で明示 | IA §5 (IA-OBJ-002)・navigation.md §6/§7・NAV-004/005・accommodation-search.md §6/§8/§10 | 残さない (既定義。正式承認は別) |
...
- **前提条件**: 本 §8F の記録が Repository に存在すること。T5 着手前に T5 の**影響度判定 (Q5)** を Web部責任者から取得すること (ただしルール制定段階で軽微でなければ高が既定)。
- **必要な判断主体**: T5 の影響度判定 = Web部責任者 (規則 §8)。上流 (表示項目・料金・評価仕様) の判断 = 影響度別 (規則 §12)。
- **必要レビュー主体**: 影響度・高につき Web部責任者 ＋ チーフデザイナー (規則 §10)。
