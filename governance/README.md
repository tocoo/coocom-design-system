...
- [owner-decisions.md](owner-decisions.md) — オーナー確認事項 (分類C) の一覧。既存資産として移行済。§4 に Review / Approval Rules 作成前に必要な確認事項を独立節として記録し、**2026-07-17 に全 11 項目のオーナー明示的回答を「オーナー回答」列へ記録済み**（残る条件・未定は「残る Open Issue」列へ分離）。**§5 に設計承認ログ (Design Approval Log) を §1〜§4 と分離して追加し、Task 009-4 (2026-07-17) の Review / Approval Rules 承認を記録**（設計承認の直接証拠の正本。GitHub merge を設計承認と同一視しない）。**§6 に適用開始の事実記録 (Activation Record) を追加し、PR #67 の main マージ（merge commit d095ded）で適用開始条件が成立した Fact を設計承認と分離して記録**（merge は設計承認ではなく適用開始条件成立の Fact）。**§7 に Travel 上流 Open Issue 解決単位の案件別判断記録を §1〜§6 と分離して追加し、Web部責任者の判断「論点別」(2026-07-21, Task 009-8) を Review / Approval Rules §12・§21② に基づくプロセス判断として記録**（設計承認ではなく、§5 設計承認ログへは追加しない）。**§8 に Travel 上流 Open Issue 初回着手論点の案件別判断記録を追加し、Web部責任者の判断「どれからでも構わない」(2026-07-21, Task 009-9。特定の 1 件に限定せず T1〜T9 のいずれからでも着手してよい) を Review / Approval Rules §8・§12 に基づくプロセス判断として記録**（設計承認ではなく、選択されなかった論点の却下・不要・継続保留ではない。§7 の「解決単位＝論点別」は上書きしない）。**§9 に Review / Approval Rules 本体の改定 (§8 編集的訂正 carve-out の追加) に対する Web部責任者の承認 (2026-07-21) を、§5 初回承認・§6 適用開始と分離して記録**。**§10 以降 (現在 §32 まで) は案件別の記録**であり、各節に ①現在判断／改訂着手の設計承認 (Review / Approval Rules §9・§20)、②影響度判定 (同 §8)、③当該案件が決定しない事項 (Does Not Decide / Does Not Authorize) を分けて記録する。対象は国内宿泊 (travel) の Design System 改定が中心で、国内レンタカー (rental-car) は §25・§29、Governance 横断ルールの受理・分類は §23 にある。**案件別の記録 (§10 以降) は、承認済みの [review-approval-rules.md](review-approval-rules.md) 本体を改定せず、Travel Work Order 6 の総合判断 (全 12 候補「現時点では開始できない」) も変更しない** (例: §15)。
...
- [review-approval-rules.md](review-approval-rules.md) (承認済み・適用中) — レビュー・承認規則の正本。owner-decisions.md §4 の 11 件のオーナー回答を根拠に構造化し、Task 009-3 で正本候補 Draft を作成、**Task 009-4 (2026-07-17) でオーナー (Web部責任者) が Repository 横断規則として明示的に承認**、**PR #67 の main マージ（merge commit d095ded）で適用開始条件が成立し適用中**となった (Status = 承認済み・適用中)。承認の直接証拠は [owner-decisions.md](owner-decisions.md) §5 設計承認ログ、適用開始の事実記録は §6。影響度の判定主体は Web部責任者の都度判断として定義済み、明文の判定基準は承認後も残る Open Issue、Alignment 完了条件 (§19 6 観点) は承認済み。Work Order 6 は Task 009-6 (2026-07-21) で再評価済み ([../services/travel/design-system/alignment-amendment-readiness-reassessment.md](../services/travel/design-system/alignment-amendment-readiness-reassessment.md), Draft): 旧共通阻害 Fact は規則で定義された範囲において解消したが候補固有の阻害 Fact が残り全 12 候補「現時点では開始できない」・Alignment は未完了。**この 12 候補の改訂は未着手**である。適用開始後、本規則には改定が 1 件あり、**§8 へ編集的訂正 carve-out (編集的訂正の類型を影響度・低とする明文の例外) を追加する改定が 2026-07-21 に承認され、本体へ反映済み**である (承認記録は [owner-decisions.md](owner-decisions.md) §9)。**12 候補とは別経路で、案件ごとの改訂着手承認 (§9・§20) に基づく Design System の改定は継続している** (travel・rental-car。記録は [owner-decisions.md](owner-decisions.md) §10 以降)。承認・適用開始だけでは他の不足成果物は整備されない。

- [contrast-rules.md](contrast-rules.md) (Draft・内容レビュー待ち) — **コントラストの品質下限の正本**。3 サービス (`travel` / `rental-car` / `inbound`) 共通の運用規約 (P2) であり、各 DS の**値**は共有しない (P1/ADR-0022)。従前は各サービスの `design.md` が品質下限を「WCAG 2.2 AA を最低ラインとし違反値を入れない」という**二値**で持つ一方、実際には AA 未達の組み合わせを 4 件、**用途名を列挙した例外条項**で個別に運用しており、列挙に無い表示要素が現れるたびに Owner 判断を要していた。本規約は下限を**テキストの役割 1〜5** で段階化し (§4)、緩和を認める**一般条件** (§5) を定めて、各 DS の内部で判定できる状態にしたもの。用語の定義 (ウェイトの境界・条件 (a) (b) (i) (ii)・大きなテキスト相当・大きなテキスト基準) の正本でもある (§3)。**適用規格・達成レベルの正式確定・適合判定・適合宣言は行わない** (§7)。**Governance の principles 正本領域への初の追加**であり、改訂着手の設計承認と影響度判定 (高) は [owner-decisions.md](owner-decisions.md) §44 に記録している (Task 009-78・[Issue #200](https://github.com/tocoo/coocom-design-system/issues/200))。Status を「承認済み・適用中」へ移す条件は内容レビュー (Web部責任者およびチーフデザイナー = [review-approval-rules.md](review-approval-rules.md) §10) と反映確定 (同 §11)。**本規約の追加によって、下記の Repository principles の正本・命名規則の正本・ADR の正本・用語定義の正本が整備されたことにはならない。**
