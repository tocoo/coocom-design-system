...
| 2026-07-29 | Task 009-33: §2 に semantic トークン `color.text.mutedStrong` (gray.700 `#616161`・bound) の行を追加し、テキスト色の 4 段構成 (`strong` `#212121` / `body` `#424242` / `mutedStrong` `#616161` / `muted` `#9e9e9e`) と、判読性を必要とする補助情報 (補助価格・税/人数/泊数等の価格条件注記・割引前価格・購買判断や内容理解に必要な補足条件) が `color.text.mutedStrong` を使用することを [事実] として明記。`color.text.muted` (白背景 ≈2.7:1) は通常テキストに求められる 4.5:1 に達しないため判読性を要する情報には用いない旨も同時に明記した。§2 表の `color.text.muted` 行には白背景コントラスト値のみを補記 (参照先 gray.600・値 `#9e9e9e`・`bound` は不変)。未確定事項の一覧に 2 行追加 (Breadcrumb 現在地への `color.text.muted` 適用の可否・`color.text.muted` の適用可能範囲の明文化。いずれも本書では判定しない)。**primitive の色値・`color.text.muted` の値および参照先・`color.text.body` は不変**。本書は適用規格・達成レベルの正式確定・適合判定・適合宣言を行わない (適用規格・達成レベルは未確定であり [alignment-blocking-facts-resolution-plan.md](alignment-blocking-facts-resolution-plan.md) §8K の T9 分類は変更していない)。version 表記は据置き (version bump の条件は [README.md](README.md) §16 の Open Issue として未決であり、同 §13 が未決のまま version bump を確定しないと定めるため。機械的な据置きではなく当該規定に基づく判断)。本工程の影響度 = **未取得** (判定主体 = Web部責任者の都度判断 = [../../../governance/review-approval-rules.md](../../../governance/review-approval-rules.md) §8。必要レビュー主体は影響度別 = 同 §10 のため判定前は未確定)。§1・§3〜§6・§8〜§9・他の token・`$status`・Component の実装要件・rental-car / inbound の成果物は不変。新規 ADR・Decision ID・正式 Status・Phase・Gate は作成・採番・新設していない | Claude Code |
