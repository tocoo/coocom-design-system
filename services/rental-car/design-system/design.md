...
1. **[既定] neutral dark 面へ切り替える** — 背景 `color.surface.inverse` (`#212121`) + 文字 `color.text.inverse` (16.10:1)。**既存トークンの範囲で、白文字 (`color.text.inverse`) が通常テキスト基準 4.5:1 を満たす唯一の面**である (下記「A 割引率ラベルに限る白文字の例外」と同じ限定)。**濃色文字を用いる面は上表のとおり他にもある**ため、「4.5:1 を満たす面が 1 つしかない」という意味ではない
