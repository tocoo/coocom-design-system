...
- 影響度・高／低の**一般的な**明文判定基準の確定 (現時点では設けられない。§8・§21 の Open Issue。判定主体は Web部責任者の都度判断として定義済み)。ただし例外として、編集的訂正の類型を影響度・低とする明文の carve-out は §8 に定義済み (Task 009-4-F1)。この carve-out 以外の一般的な高／低の内容基準は引き続き未整備。
...
- **影響度の明文判定基準**: 高／低を分ける**一般的な**明文の内容基準は現時点では設けられない（Task 009-4 オーナー回答）。これは承認後も残る Open Issue として §21 に保持する。判定主体（Web部責任者の都度判断）が定義済みであるため、明文基準の不在は規則の運用停止要因とはしない。
- **編集的訂正の例外扱い (影響度・低)**: 上記の一般基準未整備にかかわらず、次に該当する変更は影響度を既定で**低**とする明文の例外を設ける (Task 009-4-F1、Web部責任者判断 2026-07-21)。**該当する変更**: 非文・誤字脱字・明白な文法／表記の誤りの訂正など、記述の**意味・主張・意図を変えない字句の修正**。**該当しない (通常どおり Web部責任者が都度判定する)**: 設計内容・設計判断・token 値・Component 定義・version・プロセス判断に影響する変更／文の追加・削除や記述内容・整合の実質的変更／意味・主張・意図が変わりうる変更／該当するか迷う・境界的な場合 (迷う場合は低に倒さない＝安全側で高)。Web部責任者は、本例外に該当する変更を個別に高へ引き上げることができる。本例外は本規則の適用中は有効であり、ルール制定段階においても適用する。この carve-out は編集的訂正の類型に限定され、一般的な高／低の内容基準を確定するものではない (§21)。
- 上記の編集的訂正 carve-out を除き、「複数サービスに及ぶなら高」等のその他の補完ルールは追加しない (オーナー回答に存在しない)。編集的訂正 carve-out 内の「迷う場合は低に倒さない (安全側で高)」は、当該 carve-out の適用境界を定める限定的な規定であり、一般的な影響度判定への「迷ったら高」補完ルールの新設ではない。
...
- **影響度・高／低の一般的な明文判定基準は現時点では設けられない** (§8)。判定主体は Web部責任者の都度判断として定義済みだが、一般的な明文の内容基準は未整備であり承認後も残る。判定基準を本規則で推測・補完しない。**部分的解消 (Task 009-4-F1, 2026-07-21)**: 編集的訂正 (非文・誤字脱字・明白な文法／表記誤りの訂正で意味・主張・意図を変えないもの) の類型を影響度・低とする明文の carve-out を §8 に定義した。これは当該類型に限定した部分的な明文化であり、一般的な高／低の内容基準の確定ではない (一般基準は引き続き Open Issue)。
...
| 2026-07-21 | Task 009-4-F1: §8 に**編集的訂正の影響度・低 carve-out**を追加。非文・誤字脱字・明白な文法／表記の誤りの訂正で記述の意味・主張・意図を変えないものを既定で影響度・低とする明文の例外を設けた (Web部責任者判断 2026-07-21、本改定自体の影響度=高)。設計内容・設計判断・token・Component・version・プロセス判断に影響する変更、文の追加削除・記述内容/整合の実質的変更、意味が変わりうる変更、境界的な場合は本例外の対象外とし通常どおり都度判定 (迷う場合は安全側で高)。Web部責任者は個別に高へ引き上げ可能。本例外はルール制定段階を含め適用中は有効。§5 Out of Scope・§17 (手順6 の初回実施)・§21 (一般基準は Open Issue のまま・当該 carve-out は部分的解消) を整合更新。承認記録は [owner-decisions.md](owner-decisions.md) §9。規則本文の他の責務 (§7・§9〜§16)・承認内容・§18 再評価規則・§19 Alignment 完了条件・Status (承認済み・適用中) は不変。一般的な高／低の内容基準・個別変更の影響度判定そのものは確定しない。他サービス・他類型へ一般化しない | 承認済み・適用中 |
