...
- コントラストの品質下限: 下限の決め方は [../../../governance/contrast-rules.md](../../../governance/contrast-rules.md) が正本 (3 サービス共通の運用規約 = P2)。テキストの**役割**で下限が決まる — 役割 1 必須情報 = 4.5:1 (同規約 §6 の個別例外条項として本書に明記したものを除く)／役割 2 大きなテキスト相当 = 3:1／役割 3 冗長表現 = 3:1 (AA 未達を明示・一般条件は同規約 §5)／役割 4 非テキスト UI 要素 = 3:1／役割 5 装飾・非情報 = 下限なし。**本 DS は Draft であり、個々の組み合わせの実測・検証は未実施**である (未確定事項の一覧)
